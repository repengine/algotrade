...
class TestDataHandler:
    ...
    def test_cache_path_generation(self, data_handler):
        ...
        assert cache_path.name == "SPY_1d_2023-01-01_2023-12-31.parquet"
    ...
    def test_cache_functionality(self, data_handler, tmp_path, sample_ohlcv_data):
        ...
        # Mock the adapter
        data_handler.adapters["yfinance"] = type(
            "MockAdapter", (), {"fetch_ohlcv": lambda *args: sample_ohlcv_data}
        )
        ...
        cache_files = list(Path(data_handler.cache_dir).glob("*.parquet")) + list(
            Path(data_handler.cache_dir).glob("*.pkl")
        )
    ...
    def test_multi_provider_fallback(self, data_handler):
        ...
        df = data_handler.get_historical(
            "TEST", datetime.now(), datetime.now(), provider="alpha_vantage"
        )
        assert not df.empty
        assert len(df) == 3
