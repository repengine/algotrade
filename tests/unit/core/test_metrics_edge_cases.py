...
import pytest
from core.metrics import MetricsCollector, Trade
...
class TestMetricsEdgeCases:
    ...
    def test_import_fallback_scenario(self):
        ...
        original_module = sys.modules.get('core.backtest_metrics')

        try:
            # Remove backtest_metrics from modules to simulate it not existing
            if 'core.backtest_metrics' in sys.modules:
                del sys.modules['core.backtest_metrics']
            ...
            if original_module is not None:
                sys.modules['core.backtest_metrics'] = original_module
