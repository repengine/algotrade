...
class TestPerformanceBenchmarks:
    ...
    def test_data_processing_throughput(self, benchmark):
        ...
        # Benchmark data validation (simple validation)
    ...
    def test_order_processing_latency(self, benchmark):
        ...
        """
        PortfolioEngine({"initial_capital": 1000000})
        EnhancedRiskManager({})
        ...
        for i, order in enumerate(orders):
            order.order_id = f"ORDER_{i}"  # Set order ID
            start = time.perf_counter()
            order_manager.orders[order.order_id] = order  # Direct insertion for benchmark
            elapsed = time.perf_counter() - start
    ...
    def test_risk_calculation_overhead(self, benchmark):
        ...
        for _ in range(100):
            start = time.perf_counter()
            risk_manager.calculate_var(returns_data['STOCK_0'], 0.95, portfolio.total_value)
            elapsed = time.perf_counter() - start
        ...
        positions = [f"STOCK_{i}" for i in range(50)]
        weights = np.ones(50) / 50  # Equal weights

        for _ in range(50):
            start = time.perf_counter()
            risk_manager.calculate_correlation_risk(corr_matrix, positions, weights)
            elapsed = time.perf_counter() - start
        ...
        # Create market volatilities
        market_vols = {f"STOCK_{i}": np.random.uniform(0.1, 0.3) for i in range(50)}

        # Create positions dict from portfolio
        positions_dict = {
            symbol: {'value': pos.market_value, 'quantity': pos.quantity}
            for symbol, pos in portfolio.positions.items()
        }

        for _ in range(100):
            start = time.perf_counter()
            risk_manager.calculate_portfolio_risk(positions_dict, market_vols)
            elapsed = time.perf_counter() - start
