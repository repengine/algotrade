...
class TestIBKRExecutor:
    ...
    def mock_ibkr_adapter(self):
        ...
        adapter.search_contracts = AsyncMock(return_value=[
            MagicMock(conid=12345, symbol="AAPL")
        ])
        adapter.place_order = AsyncMock(return_value={"order_id": "IBKR-001"})
        adapter.cancel_order = AsyncMock(return_value=True)
        adapter.get_positions = AsyncMock(return_value=[])
        adapter.get_account_info = AsyncMock(return_value=MagicMock(
            account_id="DU123456",
    ...
    async def test_connect_with_mock(self, mock_ibkr_adapter):
        ...
        with patch("algostack.adapters.ibkr_executor.IBKRAdapter", return_value=mock_ibkr_adapter):
