...
class TestMeanReversionEquity:
    ...
    def test_position_sizing(self, strategy):
        ...
        assert position_size * signal.price <= risk_context.account_equity * risk_context.max_position_size
        
        # Check stop loss is below entry
        assert stop_loss < signal.price
        assert stop_loss == signal.price - (strategy.config['stop_loss_atr'] * signal.atr)
