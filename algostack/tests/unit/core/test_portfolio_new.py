...
class TestRiskLimits:
    ...
    def test_correlation_limit_check(self, portfolio):
        ...
        portfolio.positions["SPY"] = Position(
            ...
            quantity=100, entry_price=450.0, entry_time=datetime.now(),
            current_price=450.0
        )
        portfolio.positions["QQQ"] = Position(
            symbol="QQQ", strategy_id="test", direction="LONG",
            quantity=100, entry_price=380.0, entry_time=datetime.now(),
            current_price=380.0
        )
        ...
        assert not is_valid
        assert len(violations) == 1
        assert "SPY-QQQ correlation 0.85 exceeds limit 0.70" in violations[0]
...
class TestPortfolioVolatility:
    ...
    def test_update_correlation_matrix(self, returns_data):
        ...
        assert all(corr.diagonal() == 1.0)  # Diagonal should be 1
