...
class TestLiveTradingEngine:
    ...
    def test_mode_validation(self):
        ...
        for mode in ['paper', 'live', 'hybrid']:
            config = {'mode': mode}
            with patch('algostack.core.live_engine.PortfolioEngine'):
                with patch('algostack.core.live_engine.RiskManager'):
                    with patch('algostack.core.live_engine.DataHandler'):
                        with patch('algostack.core.live_engine.EnhancedOrderManager') as mock_order_mgr:
                            mock_order_mgr.return_value.set_active_executor = Mock()
                            with patch('algostack.core.live_engine.MetricsCollector'):
                                with patch('algostack.core.live_engine.PaperExecutor'):
                                    with patch('algostack.core.live_engine.IBKRExecutor'):
                                        engine = LiveTradingEngine(config)
