...
from algostack.adapters.ibkr_adapter import ConnectionState, IBKRWebSocketClient
...
class TestIBKRWebSocketReconnection:
    ...
    async def test_connection_state_tracking(self, ws_client):
        ...
        with patch('algostack.adapters.ibkr_adapter.ClientSession') as mock_session_class:
    ...
    async def test_receive_messages_handles_disconnection(self, ws_client):
        ...
        # Run receive loop
        receive_task = asyncio.create_task(ws_client._receive_messages())
        await asyncio.sleep(0.1)
