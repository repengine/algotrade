...
from algostack.core.engine.order_manager import Order, OrderManager, OrderSide, OrderType
from algostack.core.portfolio import PortfolioEngine
from algostack.core.risk import EnhancedRiskManager
from algostack.core.trading_engine_main import TradingEngine
...
class TestPerformanceBenchmarks:
    ...
    def test_risk_calculation_overhead(self, benchmark):
        ...
        for _ in range(100):
            start = time.perf_counter()
            risk_manager.calculate_var(portfolio, returns_data, confidence=0.95)
            elapsed = time.perf_counter() - start
        ...
        for _ in range(50):
            start = time.perf_counter()
            risk_manager.calculate_correlation_matrix(returns_data)
            elapsed = time.perf_counter() - start
        ...
        for _ in range(100):
            start = time.perf_counter()
            risk_manager.calculate_portfolio_risk(portfolio)
            elapsed = time.perf_counter() - start
