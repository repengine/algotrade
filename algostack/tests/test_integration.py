...
class TestSystemIntegration:
    ...
    def strategies(self, system_config):
        ...
        return {
            'mean_reversion': MeanReversionEquity(system_config['strategies']['mean_reversion']),
            'trend_following': TrendFollowingMulti(system_config['strategies']['trend_following'])
        }
    
    def test_signal_generation_to_execution(self, portfolio_engine, strategies, mock_market_data):
        ...
        if signals:
            ...
            for signal in signals:
                if signal.strategy_id in allocations:
                    ...
                    position_size, stop_loss = portfolio_engine.size_position(signal, allocation)
                    
                    if position_size > 0:
                        position = portfolio_engine.execute_signal(signal, position_size, stop_loss)
    ...
    def test_global_risk_shutdown(self, portfolio_engine):
        ...
        assert portfolio_engine.is_risk_off == True
        assert portfolio_engine.risk_off_until is not None
    
    def test_multi_strategy_coordination(self, portfolio_engine, strategies, mock_market_data):
        ...
        for day in range(10):
            ...
            if all_signals:
                allocations = portfolio_engine.allocate_capital(all_signals, mock_market_data)
    ...
    def test_full_system_stress_test(self, portfolio_engine, risk_manager, strategies):
        ...
        for _ in range(100):
            ...
            # Process signal
            if signal.direction == 'FLAT' and signal.symbol in portfolio_engine.positions:
                portfolio_engine.close_position(signal.symbol, signal.price)
            ...
            if np.random.random() > 0.5:
                ...
                for exit_signal in exit_signals:
                    portfolio_engine.close_position(exit_signal.symbol, exit_signal.price)
