...
class DebugOvernightDrift(BaseOvernightDrift):
    ...
    def next(self, data: pd.DataFrame) -> Optional[Signal]:
        ...
        if symbol in self.positions:
            print(f"✓ Have existing position")
        ...
        if self.is_blackout_period(symbol, current_date):
            print(f"❌ In blackout period")
            return None
        
        # Volatility filter
        print(f"  ATR: {latest['atr']:.4f} (range: {self.config['min_atr']}-{self.config['max_atr']})")
        if latest['atr'] < self.config['min_atr'] or latest['atr'] > self.config['max_atr']:
            print(f"❌ ATR out of range")
            return None
            
        # Volume filter
        print(f"  Volume ratio: {latest['volume_ratio']:.2f} (threshold: {self.config['volume_threshold']})")
        ...
        if self.config['trend_filter']:
            ...
            if latest['close'] < latest['sma']:
                print(f"❌ Below SMA (trend filter)")
                return None
        ...
        if edge > 0.02:
            print(f"✓ Edge is positive, creating signal...")
