...
from core.trading_engine_main import EngineConfig, TradingEngine
...
def backtest(
    strategy: str, start: str, end: str, config: str, walk_forward: bool
) -> None:
    ...
    if walk_forward:
        ...
        results_df = run_walk_forward_optimization(
            MeanReversionEquity, strategy_config["params"], symbols, start, end
        )
...
def fetch_data(symbol: str, start: str, provider: str) -> None:
    ...
    df = handler.get_historical(
        symbol, datetime.strptime(start, "%Y-%m-%d"), datetime.now()
    )
