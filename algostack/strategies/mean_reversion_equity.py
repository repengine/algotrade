...
class MeanReversionEquity(BaseStrategy):
    ...
    def calculate_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        df['sma_exit'] = talib.SMA(df['close'], timeperiod=self.config['ma_exit_period'])
    ...
    def next(self, data: pd.DataFrame) -> Optional[Signal]:
        ...
        if symbol in self.positions:
            ...
            # 2. Stop loss
            stop_price = pos['entry_price'] - (self.config['stop_loss_atr'] * pos['entry_atr'])
            ...
            if exit_signal:
                signal = Signal(
                    ...
                    metadata={
                        ...
                        'exit_trigger': 'price_above_ma' if latest['close'] > latest['sma_exit'] else 'stop_loss',
        ...
        # Check for entry signals
            ...
            if oversold and below_band and volume_confirm:
                ...
                strength = min(1.0, (self.config['rsi_oversold'] - latest['rsi']) / self.config['rsi_oversold']) if self.config['rsi_oversold'] > 0 else 0.5
                ...
                signal = Signal(
                    ...
                    metadata={
                        ...
                        'rsi': latest['rsi'],
                        'band_distance': (latest['lower_band'] - latest['close']) / latest['atr'],
