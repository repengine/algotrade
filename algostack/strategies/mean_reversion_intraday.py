...
class MeanReversionIntraday(BaseStrategy):
    ...
    def next(self, data: pd.DataFrame) -> Optional[Signal]:
        ...
        if symbol in self.positions:
            ...
            # 4. End of day exit
            if hasattr(current_time, 'hour') and current_time.hour >= 15 and current_time.minute >= 45:
                exit_signal = True
            ...
            if exit_signal:
                signal = Signal(
                    ...
                    metadata={
                        ...
                        'rsi': latest['rsi'],
                        'bars_held': bars_held,
                        'pnl_pct': ((latest['close'] - pos['entry_price']) / pos['entry_price'] * 100)
                    }
                )
        ...
        # Check for entry signals
            ...
            if zscore_oversold and rsi_oversold and volume_confirm:
                ...
                zscore_strength = min(1.0, abs(latest['zscore'] + self.config['zscore_threshold']) / self.config['zscore_threshold'])
                rsi_strength = min(1.0, (self.config['rsi_oversold'] - latest['rsi']) / self.config['rsi_oversold'])
    ...
    def backtest_metrics(self, trades: pd.DataFrame) -> dict:
        ...
        if 'entry_time' in trades.columns and 'exit_time' in trades.columns:
            ...
            hour_distribution = trades.groupby('entry_hour')['pnl_pct'].agg(['count', 'mean'])
            metrics['best_hour'] = hour_distribution['mean'].idxmax() if not hour_distribution.empty else None
