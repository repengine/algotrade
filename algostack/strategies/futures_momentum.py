...
class FuturesMomentum(BaseStrategy):
    ...
    def next(self, data: pd.DataFrame) -> Optional[Signal]:
        ...
        if hasattr(current_time, 'hour'):
            ...
            if hour < self.config['trade_hours']['start'] or hour > self.config['trade_hours']['end']:
                return None
        
        # Calculate indicators
        high_lookback = data['high'].iloc[-self.config['lookback_period']:].max()
        low_lookback = data['low'].iloc[-self.config['lookback_period']:].min()
        ...
        if symbol in self.positions and self.positions[symbol] > 0:
            ...
            elif hasattr(current_time, 'hour') and current_time.hour >= 15 and current_time.minute >= 30:
                # End of day exit
                self.positions[symbol] = 0
                return Signal(
                    symbol=symbol,
            ...
            breakout_level = high_lookback * (1 + self.config['breakout_threshold'] / 100)
            
            # Check for momentum breakout
            if (current_price > breakout_level and
                current_rsi > self.config['rsi_threshold'] and
                current_volume_ratio > self.config['volume_multiplier'] and
                not pd.isna(current_rsi) and not pd.isna(current_atr)):
                
                # Calculate position details
                stop_distance = current_atr * self.config['stop_loss_atr']
                stop_price = current_price - stop_distance
                target_price = current_price + (current_atr * self.config['profit_target_atr'])
    ...
    def backtest_metrics(self, trades_df: pd.DataFrame) -> dict[str, float]:
        ...
        metrics = {
            ...
            'avg_bars_in_trade': trades_df['bars_in_trade'].mean() if 'bars_in_trade' in trades_df else 0
        }
