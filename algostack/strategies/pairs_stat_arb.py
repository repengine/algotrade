...
class PairsStatArb(BaseStrategy):
    ...
    def find_cointegrated_pairs(
        ...
        for i, sym1 in enumerate(symbols):
            for _j, sym2 in enumerate(symbols[i + 1 :], i + 1):
    ...
    def _calculate_spread(
        self, prices1: pd.Series, prices2: pd.Series, window: Optional[int] = None
    ) -> pd.Series:
    ...
    def _calculate_zscore(
        self, spread: pd.Series, window: Optional[int] = None
    ) -> pd.Series:
    ...
    def next_pairs(self, price_data: dict[str, pd.DataFrame]) -> list[Signal]:
        ...
        for (sym1, sym2), _pair_info in self.pairs.items():
            if sym1 not in price_data or sym2 not in price_data:
                continue
            ...
            if pair_key in self.positions:
                ...
                if pos["direction"] == "LONG_SPREAD":
                    ...
                    if (
                        abs(latest_zscore) < self.config["exit_threshold"]
                        or latest_zscore < -self.config["stop_threshold"]
                    ):
                        exit_signal = True
