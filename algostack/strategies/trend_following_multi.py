...
class TrendFollowingMulti(BaseStrategy):
    ...
    def calculate_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        df['channel_position'] = np.where(channel_width > 0, (df['close'] - df['channel_low']) / channel_width, 0.5)
    ...
    def next(self, data: pd.DataFrame) -> Optional[Signal]:
        ...
        if symbol in self.positions:
            ...
            if exit_signal:
                pnl_pct = 0
                if pos['direction'] == 'LONG':
                    pnl_pct = ((latest['close'] - pos['entry_price']) / pos['entry_price'] * 100) if pos['entry_price'] > 0 else 0.0
                else:  # SHORT
                    pnl_pct = ((pos['entry_price'] - latest['close']) / pos['entry_price'] * 100) if pos['entry_price'] > 0 else 0.0
        ...
        # Check for entry signals (if not in position)
            ...
            if self.config['use_volume_filter']:
                volume_confirm = latest['volume_ratio'] > self.config['volume_threshold']
            
            # Long entry: breakout above channel high
            if (latest['close'] > latest['channel_high'] and 
                prev['close'] <= prev['channel_high'] and
                strong_trend and
                latest['plus_di'] > latest['minus_di'] and
                volume_confirm):
                
                # Calculate signal strength based on breakout magnitude and trend strength
                breakout_strength = min(1.0, (latest['close'] - latest['channel_high']) / latest['atr']) if latest['atr'] > 0 else 0.5
            ...
            elif (latest['close'] < latest['channel_low'] and 
                  prev['close'] >= prev['channel_low'] and
                  strong_trend and
                  latest['minus_di'] > latest['plus_di'] and
                  volume_confirm):
                
                # Calculate signal strength
                breakout_strength = min(1.0, (latest['channel_low'] - latest['close']) / latest['atr']) if latest['atr'] > 0 else 0.5
                trend_strength = min(1.0, latest['adx'] / 40)
                strength = -(breakout_strength + trend_strength) / 2  # Negative for short
    ...
    def size(self, signal: Signal, risk_context: RiskContext) -> tuple[float, float]:
        ...
        vol_weight = min(1.0, risk_context.volatility_target / annualized_vol) if annualized_vol > 0 else 0.5
        ...
        max_position_value = risk_context.account_equity * risk_context.max_position_size
    ...
    def backtest_metrics(self, trades: pd.DataFrame) -> dict:
        ...
        if not long_trades.empty:
            ...
            metrics['long_win_rate'] = len(long_wins) / len(long_trades) if len(long_trades) > 0 else 0.0
        ...
        if not short_trades.empty:
            ...
            metrics['short_win_rate'] = len(short_wins) / len(short_trades) if len(short_trades) > 0 else 0.0
