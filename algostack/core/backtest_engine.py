...
class TransactionCostModel:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.commission_type = config.get(
            "commission_type", "per_share"
        )  # per_share, percentage

        # Spread modeling
        self.spread_model = config.get(
            "spread_model", "fixed"
        ...
        self.urgency_factor = config.get(
            "urgency_factor", 1.0
        )  # 1.0 = normal, >1 = urgent
...
class WalkForwardAnalyzer:
    ...
    def run_analysis(
        ...
        if len(windows) > self.max_windows:
            logger.warning(
                f"Limiting walk-forward from {len(windows)} to {self.max_windows} windows"
            )
        ...
        for i, (is_start, is_end, oos_start, oos_end) in enumerate(windows):
            logger.info(
                f"Window {i+1}/{len(windows)}: IS {is_start.date()} to {is_end.date()}, "
            ...
            logger.info(
                f"  Optimizing {len(param_grid)} parameters on {len(is_data)} days of IS data..."
            )
            ...
            try:
                ...
                oos_results = backtest_func(
                    oos_strategy, oos_data, cost_model=cost_model
                )
            ...
            results.append(
                {
                    ...
                    "is_sharpe": (
                        optimal_params.get("_is_sharpe", 0) if optimal_params else 0
                    ),
    ...
    def _generate_windows(self, data: pd.DataFrame) -> Generator[tuple[pd.Timestamp, pd.Timestamp, pd.Timestamp, pd.Timestamp], None, None]:
        ...
        while (
            current_start + pd.Timedelta(days=self.window_size + self.step_size)
            <= data_end
        ):
    ...
    def _optimize_parameters(
        ...
        param_grid: dict[str, list[Any]],
        backtest_func: Callable,
        cost_model: Optional[TransactionCostModel],
    ) -> dict[str, Any]:
        ...
        for values_tuple in itertools.product(*param_values):
            ...
            if combination_count % max(1, total_combinations // 4) == 0:
                ...
                logger.info(
                    f"Optimization progress: {progress:.0f}% ({combination_count}/{total_combinations} combinations)"
                )
...
class MonteCarloValidator:
    ...
    def validate_strategy(
        ...
        # Calculate p-value
        p_value = np.sum(np.array(random_sharpes) >= actual_sharpe) / len(
            random_sharpes
        )
    ...
    def _simulate_random_entries(self, returns: pd.Series) -> list[float]:
        ...
        for _ in range(self.n_simulations):
            ...
            if len(shuffled_returns) > 1 and np.std(shuffled_returns) > 0:
                sharpe = (
                    np.sqrt(252)
                    * float(np.mean(shuffled_returns))
                    / float(np.std(shuffled_returns))
                )
    ...
    def _compare_to_benchmark(
        ...
        # Align series
        aligned_strat, aligned_bench = strategy_returns.align(
...
class RegimeAnalyzer:
    ...
    def _volatility_regimes(self, data: pd.DataFrame) -> dict[str, pd.DataFrame]:
        ...
        regimes = {
            ...
            "medium_volatility": data[
                (volatility > vol_percentiles.iloc[0])
    ...
    def analyze_regime_performance(
        ...
        sharpe_values = [
            r["sharpe"] for r in results.values() if r["sharpe"] is not None
        ]

        if len(sharpe_values) > 1:
            consistency_score = 1 - (
                np.std(sharpe_values) / (np.mean(sharpe_values) + 1e-6)
            )
...
def create_backtest_report(results: dict[str, Any]) -> str:
    ...
    if "regime_analysis" in results:
        ...
        report.append(
            f"- Positive in All Regimes: {'Yes' if regime['all_positive'] else 'No'}"
        )
        report.append("")
        ...
        for regime_name, metrics in regime["regime_results"].items():
            report.append(
                f"- {regime_name}: Sharpe={metrics['sharpe']:.2f}, "
