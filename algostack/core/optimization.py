...
class PlateauDetector:
    ...
    def find_plateaus(self, results_df: pd.DataFrame, metric_col: str = 'sharpe') -> List[Dict[str, Any]]:
        ...
        param_cols = [col for col in results_df.columns if col not in [metric_col, 'value']]
        ...
        for i, is_flat in enumerate(flat_mask):
            if is_flat:
                ...
                if len(current_plateau) >= self.min_plateau_size:
                    ...
                    plateaus.append({
                        'param_range': {param_col: (params[plateau_indices[0]], 
                                                   params[plateau_indices[-1]])},
                        'center': {param_col: params[plateau_indices[len(plateau_indices)//2]]},
                        'size': len(plateau_indices),
                        'mean_value': np.mean(values[plateau_indices]),
                        'std_value': np.std(values[plateau_indices]),
                        'stability': 1.0 - np.std(values[plateau_indices]) / (np.mean(values[plateau_indices]) + 1e-6)
                    })
                current_plateau = []
        ...
        for label_id in range(1, num_features + 1):
            ...
            if plateau_size >= self.min_plateau_size:
                ...
                plateaus.append({
                    ...
                    'stability': 1.0 - np.nanstd(grid[plateau_mask]) / (np.nanmean(grid[plateau_mask]) + 1e-6)
                })
    ...
    def _find_nd_plateaus(self, df: pd.DataFrame, param_cols: List[str], metric_col: str) -> List[Dict[str, Any]]:
        ...
        metric_scaled = (df[metric_col].values - df[metric_col].mean()) / df[metric_col].std()
        X_with_metric = np.column_stack([X_scaled, metric_scaled * 2])  # Weight metric dimension
        ...
        for label in set(labels):
            ...
            if len(cluster_df) >= self.min_plateau_size:
                ...
                plateaus.append({
                    ...
                    'stability': 1.0 - cluster_df[metric_col].std() / (cluster_df[metric_col].mean() + 1e-6)
                })
...
class CoarseToFineOptimizer:
    ...
    def optimize(self,
        ...
        if not plateaus:
            ...
            best_params = coarse_results.loc[best_idx, [col for col in coarse_results.columns if col != 'value']].to_dict()
        ...
        logger.info(f"Found {len(plateaus)} plateaus, refining search in best plateau...")
        best_plateau = max(plateaus, key=lambda p: p['mean_value'] * p['stability'])
        
        fine_results = self._fine_search(objective_func, best_plateau['param_range'], n_jobs)
    ...
    def _coarse_search(self, 
        ...
        for param_name, (min_val, max_val) in param_ranges.items():
            if isinstance(min_val, (int, float)):
                param_grids[param_name] = np.linspace(min_val, max_val, self.coarse_grid_points)
            else:
    ...
    def _fine_search(self,
        ...
        for param_name, (min_val, max_val) in param_ranges.items():
            if isinstance(min_val, (int, float)):
                param_grids[param_name] = np.linspace(min_val, max_val, self.fine_grid_points)
            else:
                param_grids[param_name] = [min_val, max_val]
                
        # Rest is same as coarse search
        return self._coarse_search(objective_func, {k: (v[0], v[-1]) for k, v in param_grids.items()}, n_jobs)
...
class BayesianOptimizer:
    ...
    def optimize(self,
        ...
        if multi_objective:
            ...
            acceptable_trials = [t for t in best_trials if t.values[1] < 0.20]  # Max 20% drawdown
        ...
        for param_name, param_value in best_params.items():
            if f'params_{param_name}' in top_trials.columns:
                ...
                if top_trials[param_col].dtype in [np.float64, np.int64]:
                    ...
                    param_range = top_trials[param_col].max() - top_trials[param_col].min()
                    if param_range > 0:
...
class EnsembleOptimizer:
    ...
    def create_ensemble(self, optimization_result: OptimizationResult) -> List[Dict[str, Any]]:
        ...
        param_cols = [col for col in results_df.columns if col not in ['value', 'trial_id']]
        ...
        while len(ensemble) < self.n_ensemble and len(candidates) > 0:
            ...
            for _, candidate in candidates.iterrows():
                ...
                min_distance = min(
                    self._param_distance(candidate_params, e)
                    for e in ensemble
                )
                
                # Combined score: performance + diversity
                combined_score = (
                    (1 - self.diversity_weight) * candidate['value'] +
...
def create_optuna_objective(strategy_class,
                          data_train: pd.DataFrame,
                          data_val: pd.DataFrame,
                          backtest_func: Callable,
                          cost_model: Optional[Any] = None,
                          stability_penalty: float = 0.1) -> Callable:
    ...
    def objective(trial):
        ...
        decay = (train_results['sharpe_ratio'] - val_results['sharpe_ratio']) / (abs(train_results['sharpe_ratio']) + 1e-6)
