...
class ExecutionHandler:
    ...
    async def execute_order(self, order: Order, params: Optional[ExecutionParams] = None) -> ExecutionPlan:
        ...
        self.logger.info(f"Started {params.algorithm.value} execution for order {order.order_id}")
        return plan
    ...
    async def _execute_twap(self, plan: ExecutionPlan) -> None:
        ...
        try:
            ...
            num_slices = max(10, int(duration / 60))  # At least 10 slices or 1 per minute
            slice_quantity = order.quantity / num_slices
            slice_interval = duration / num_slices
            
            self.logger.info(f"TWAP execution: {num_slices} slices of {slice_quantity:.2f} every {slice_interval:.0f}s")
    ...
    async def _execute_iceberg(self, plan: ExecutionPlan) -> None:
        ...
        try:
            ...
            visible_quantity = params.slice_size or (order.quantity * 0.1)  # 10% visible by default
            hidden_quantity = params.hidden_quantity or (order.quantity - visible_quantity)
    ...
    async def _execute_pov(self, plan: ExecutionPlan) -> None:
        ...
        try:
            ...
            while remaining > 0 and plan.status == "executing":
                ...
                market_volume = await self._get_market_volume(order.symbol, lookback_seconds=60)
                
                # Calculate our slice based on participation rate
                slice_size = min(
                    market_volume * params.max_participation_rate,
                    remaining
                )
    ...
    async def _wait_for_order_completion(self, order: Order, timeout: float) -> None:
        ...
        while order.is_active() and (datetime.now() - start_time).total_seconds() < timeout:
    ...
    def get_execution_stats(self, order_id: str) -> Optional[Dict[str, Any]]:
        ...
        if total_filled > 0:
            avg_price = sum(o.average_fill_price * o.filled_quantity for o in filled_orders) / total_filled
        else:
            avg_price = 0.0
        ...
        return {
            ...
            "duration_seconds": (plan.completion_time - plan.start_time).total_seconds() if plan.completion_time else None
        }