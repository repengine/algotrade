...
def test_signals():
    ...
    if buy_signal.sum() > 0:
        ...
        print(
            f"Date: {buy_dates[0]}, Price: ${data.loc[buy_dates[0], 'Close']:.2f}, RSI: {rsi.loc[buy_dates[0]]:.2f}"
        )
    ...
    if bullish_cross.sum() > 0:
        ...
        print(
            f"Fast MA: ${fast_ma.loc[bull_dates[0]]:.2f}, Slow MA: ${slow_ma.loc[bull_dates[0]]:.2f}"
        )
