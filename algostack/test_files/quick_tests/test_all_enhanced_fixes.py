...
print(
    f"   Constant returns: {'✅ PASS' if 'confidence_interval' in result else '❌ FAIL'}"
)
...
bad_wf_results = pd.DataFrame(
    {"window_num": [1], "message": ["Walk-forward failed: test error"]}
)
...
print(
    f"   Expected columns available: {len(available_cols)}/{len(expected_cols)} {'✅ PASS' if len(available_cols) == len(expected_cols) else '❌ FAIL'}"
)
