...
class StrategyTester:
    ...
    def test_strategy(self, strategy_class, config: Dict[str, Any], 
                     start_date: str = "2021-01-01", 
                     end_date: str = "2023-01-01") -> Dict[str, Any]:
        ...
        try:
            ...
            benchmark_monthly = test_data['close'].resample('M').last().pct_change().dropna()
    ...
    def find_winning_configs(self):
        ...
        for combo in itertools.product(*values):
            ...
            if result['success'] and result.get('all_profitable') and result.get('all_outperform'):
                logger.info(f"🎉 FOUND PERFECT MEAN REVERSION STRATEGY!")
        ...
        for combo in itertools.product(*values):
            ...
            if result['success'] and result.get('all_profitable') and result.get('all_outperform'):
                logger.info(f"🎉 FOUND PERFECT TREND FOLLOWING STRATEGY!")
                logger.info(f"Config: {config}")
                logger.info(f"Total Return: {result['total_return']:.2f}%")
...
def main():
    ...
    if winning_configs:
        ...
        logger.info("Consider expanding parameter ranges or trying different strategies.")
