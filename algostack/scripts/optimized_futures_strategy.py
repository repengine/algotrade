...
class OptimizedFuturesStrategy:
    ...
    def test_momentum_breakout(self, data):
        ...
        for i in range(30, len(df)):
            ...
            if position == 0:
                # Breakout entry
                breakout_level = df['high_20'].iloc[i-1] * (1 + config['breakout_threshold']/100)
                ...
                if price > breakout_level and df['rsi'].iloc[i] > config['rsi_confirm']:
                    ...
                    if contracts > 0:
                        ...
                        target_price = price + (stop_distance * config['profit_multiplier'])
    ...
    def test_opening_range_breakout(self, data):
        ...
        df['is_tradeable'] = (df.index.hour >= 9) & (df.index.hour < 12)  # Trade until noon
        ...
        for date, day_data in df.groupby(df.index.date):
            ...
            for idx in day_data[day_data['is_tradeable']].index:
                ...
                if position == 0:
                    # Long breakout
                    if price > range_high + (config['breakout_buffer'] * 0.25):  # 0.25 = tick size
                        position = 1
                        entry_price = price
                        stop_price = range_low - (config['stop_buffer'] * 0.25)
                        target_price = entry_price + (range_size * config['target_multiplier'])
                        entry_time = idx
                    ...
                    if position == 1:  # Long position
                        if price <= stop_price or price >= target_price or time_elapsed > config['time_limit']:
                            pnl = (price - entry_price) * 5 - 1.04  # MES P&L and commission
                            capital += pnl
