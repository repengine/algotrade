        ...
        for rsi_period in [2, 3, 5]:
            for rsi_oversold in [10, 15, 20, 25]:
                ...
                try:
                    ...
                    equity_monthly = backtest_results['equity_curve'].resample('M').last()
                    strategy_monthly_returns = equity_monthly.pct_change().dropna()
                    
                    # Check monthly criteria
                    outperformance = strategy_monthly_returns - benchmark_monthly_returns
                    ...
                    result = {
                        ...
                        'perfect': num_profitable == len(strategy_monthly_returns) and num_outperform == len(strategy_monthly_returns)
                    }
                    
                    results.append(result)
                    
                    if result['perfect']:
                        logger.info(f"🎉 PERFECT STRATEGY FOUND!")
...
def main():
    ...
    # Sort by performance
    results.sort(key=lambda x: (x['outperforming_months'], x['total_return']), reverse=True)
    ...
    if perfect_strategies:
        ...
        with open(f"WINNING_CONFIG_{timestamp}.json", 'w') as f:
            json.dump({
                ...
                'performance': {
                    ...
                    'avg_monthly_return': perfect_strategies[0]['avg_monthly_return'],
                    'avg_outperformance': perfect_strategies[0]['avg_outperformance']
                }
            }, f, indent=2)
        ...
        for i, strategy in enumerate(results[:5]):
            print(f"\n#{i+1}:")
            print(f"Strategy: {strategy['strategy']}")
            print(f"Outperforming Months: {strategy['outperforming_months']}/{strategy['total_months']}")
            print(f"Total Return: {strategy['total_return']:.2f}%")
            print(f"Sharpe Ratio: {strategy['sharpe_ratio']:.2f}")
