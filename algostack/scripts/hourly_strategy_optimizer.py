...
class HourlyStrategyOptimizer:
    ...
    def load_hourly_data(self):
        ...
        logger.info(f"Date range: {self.hourly_data.index[0]} to {self.hourly_data.index[-1]}")
        
    def test_hourly_strategy(self, strategy_class, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            min_lookback = config.get('lookback_period', 20) * 8  # Convert days to hours (8 trading hours/day)
            ...
            if len(daily_returns) > 0:
                sharpe_daily = np.sqrt(252) * daily_returns.mean() / (daily_returns.std() + 1e-6)
            else:
                sharpe_daily = 0
                
            if len(hourly_returns) > 0:
                # Annualized hourly Sharpe (1950 trading hours per year)
                sharpe_hourly = np.sqrt(1950) * hourly_returns.mean() / (hourly_returns.std() + 1e-6)
            else:
                sharpe_hourly = 0
            ...
            return {
                ...
                'trades_per_day': len(trades) / (len(self.hourly_data) / (6.5 * 5)),  # 6.5 hours per day, 5 days per week
    ...
    def optimize_hourly_strategies(self):
        ...
        hourly_params = {
            'symbols': [['SPY']],
            'lookback_period': [5, 10, 15, 20],  # Days (will be converted to hours internally)
                    ...
                    for rsi_period in hourly_params['rsi_period']:
                        for rsi_os in hourly_params['rsi_oversold']:
                            for rsi_ob in hourly_params['rsi_overbought']:
                                ...
                                result = self.test_hourly_strategy(MeanReversionEquity, config)
                                tested += 1
                                
                                if result['success']:
                                    results.append(result)
                                    
                                    if result['sharpe_hourly'] > 1.5 and result['num_trades'] > 10:
                                        logger.info(f"Good hourly strategy found!")
                                        logger.info(f"  Return: {result['total_return']:.2f}%")
                                        logger.info(f"  Hourly Sharpe: {result['sharpe_hourly']:.2f}")
