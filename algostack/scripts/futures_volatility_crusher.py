...
class FuturesVolatilityCrusher:
    ...
    def calculate_indicators(self, data):
        ...
        df['volatility_regime'] = pd.cut(df['atr'], bins=3, labels=['low', 'medium', 'high'])
    ...
    def simulate_futures_trading(self, data, initial_capital=25000):
        ...
        for i in range(30, len(df)):
            ...
            if position != 0:
                # Mark-to-market P&L
                pnl = position * (current_price - entry_price) * self.contract_specs['multiplier']
            ...
            if position == 0:
                # Entry conditions
                entry_signal = (
                    df['zscore'].iloc[i] < self.config['z_entry'] and
                    df['rsi'].iloc[i] < self.config['rsi_oversold'] and
                    df['volume'].iloc[i] > self.config['min_volume'] and
                    not pd.isna(df['zscore'].iloc[i])
                )
                ...
                if entry_signal:
                    ...
                    required_margin = contracts * self.contract_specs['margin_requirement']
                    if required_margin <= capital * 0.5:  # Use max 50% of capital
                        position = contracts
                        entry_price = current_price
                        entry_time = current_time
                        stop_price = entry_price - (self.config['stop_loss_ticks'] * self.contract_specs['tick_size'])
                        target_price = entry_price + (self.config['profit_target_ticks'] * self.contract_specs['tick_size'])
                        
                        # Commission
                        capital -= contracts * self.contract_specs['commission'] * 2  # Round trip
                        ...
                        trades.append({
                            ...
                            'session': 'US' if df['is_us_session'].iloc[i] else 
                                      'Europe' if df['is_european_session'].iloc[i] else 'Asia'
                        })
            ...
            # EXIT LOGIC
                ...
                eod_flat = df['hour'].iloc[i] == 15 and df.index[i].minute >= 45  # Flatten before close
                
                if hit_stop or hit_target or eod_flat:
                    # Calculate P&L
                    gross_pnl = position * (current_price - entry_price) * self.contract_specs['multiplier']
                    capital += gross_pnl
                    ...
                    trades[-1].update({
                        ...
                        'gross_pnl': gross_pnl,
                        'net_pnl': gross_pnl - (position * self.contract_specs['commission'] * 2),
                        'exit_reason': 'stop_loss' if hit_stop else 'target' if hit_target else 'eod_flat',
        ...
        if position != 0:
            ...
            gross_pnl = position * (final_price - entry_price) * self.contract_specs['multiplier']
            capital += gross_pnl
            trades[-1].update({
        ...
        if trades:
            ...
            for session in ['US', 'Europe', 'Asia']:
                if session in session_trades:
                    print(f"{session}: {session_trades[session]} trades, "
...
def main():
    ...
    for contract_type in ['MES', 'ES']:
        ...
        if results['trades']:
            ...
            # Capital efficiency
            margin_used = crusher.config['position_size'] * crusher.contract_specs['margin_requirement']
