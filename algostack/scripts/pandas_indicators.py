...
class PandasIndicators:
    ...
    def ADX(
        ...
        atr = tr.rolling(window=timeperiod, min_periods=1).mean()
        plus_di = 100 * (
            plus_dm.rolling(window=timeperiod, min_periods=1).mean()
            / atr.replace(0, 1e-10)
        )
        minus_di = 100 * (
            minus_dm.rolling(window=timeperiod, min_periods=1).mean()
            / atr.replace(0, 1e-10)
        )
    ...
    @staticmethod
    def OBV(
        close: Union[pd.Series, pd.DataFrame], volume: Union[pd.Series, pd.DataFrame]
    ) -> pd.Series:
        """On Balance Volume."""
        if isinstance(close, pd.DataFrame):
            close = close.iloc[:, 0]
    ...
    @staticmethod
    def STDDEV(
        close: Union[pd.Series, pd.DataFrame], timeperiod: int = 5, nbdev: float = 1
    ) -> pd.Series:
        """Standard Deviation."""
        if isinstance(close, pd.DataFrame):
            close = close.iloc[:, 0]
    ...
    @staticmethod
    def LINEARREG(
        close: Union[pd.Series, pd.DataFrame], timeperiod: int = 14
    ) -> pd.Series:
        """Linear Regression."""
        if isinstance(close, pd.DataFrame):
            close = close.iloc[:, 0]
...
def create_talib_compatible_module():
    ...
    def WMA(close, timeperiod=30):
        ...
        return close.rolling(timeperiod).apply(
