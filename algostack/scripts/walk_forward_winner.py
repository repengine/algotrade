...
from core.optimization import OptimizationDataPipeline
...
class WalkForwardWinner:
    ...
    def __init__(self, 
                 ...
                 reoptimize_frequency: int = 1):  # Reoptimize every month
        self.symbol = symbol
    ...
    def optimize_for_month(self, 
                          strategy_class,
                          train_data: pd.DataFrame,
                          test_start: datetime,
                          test_end: datetime) -> Tuple[Dict[str, Any], float]:
    ...
    def run_walk_forward(self, start_date: str, end_date: str) -> List[MonthlyResult]:
        ...
        full_data = self.data_handler.fetch_data(
            self.symbol,
            datetime.strptime(start_date, "%Y-%m-%d") - timedelta(days=365),  # Extra data for lookback
        ...
        while current_date < test_end:
            ...
            for strategy_class in strategy_classes:
                ...
                try:
                    ...
                    month_return = (results['equity_curve'].iloc[-1] / results['equity_curve'].iloc[0]) - 1
                    
                    # Calculate benchmark return
                    benchmark_return = (test_data['close'].iloc[-1] / test_data['close'].iloc[0]) - 1
                    
                    # We want to beat benchmark
                    if month_return > benchmark_return and month_return > best_month_return:
            ...
            if best_params:
                ...
                logger.info(f"Month {month_str}: Return={best_month_return:.2%}, Benchmark={best_benchmark_return:.2%}, Outperformance={result.outperformance:.2%}")
            else:
            ...
            if month_count >= 6:
                ...
                if winning_months < 4:  # Less than 67% win rate
                    logger.warning("Strategy not performing well enough, adjusting approach...")
...
def main():
    ...
    output_file = f"walk_forward_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    print(f"Profitable Months: {results['profitable_months']} ({results['profitable_months']/results['total_months']*100:.1f}%)")
    print(f"Outperforming Months: {results['outperforming_months']} ({results['win_rate']*100:.1f}%)")
    print(f"Average Monthly Return: {results['avg_monthly_return']*100:.2f}%")
    print(f"Average Monthly Outperformance: {results['avg_monthly_outperformance']*100:.2f}%")
    ...
    if results['profitable_months'] == results['total_months'] and results['outperforming_months'] == results['total_months']:
