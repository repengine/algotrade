...
class AggressiveStrategyFinder:
    ...
    def test_leveraged_mean_reversion(self, data, timeframe="daily"):
        ...
        for config in configs:
            ...
            for i in range(config["lookback"], len(data)):
                ...
                if (
                    position == 0
                    and current_z < -config["z_entry"]
                    and current_rsi < config["rsi_os"]
                ):
                    # Use leverage
                    ...
                    if shares > 0:
                        cash -= (
                            shares * current_price / config["leverage"]
                        )  # Only use actual cash
                        position = 1
        ...
        for config in configs:
            ...
            for i in range(config["bb_period"], len(data)):
                ...
                if position == 0:
                    # Bullish breakout
                    if (
                        price > upper_band.iloc[i]
                        and rsi.iloc[i] > config["rsi_threshold"]
                    ):
                        # Risk 2% per trade
                        ...
                        if shares * price < cash * 0.95:
                            ...
                            target_price = price + (
                                atr.iloc[i] * config["profit_target"]
                            )
            ...
            days = (data.index[-1] - data.index[0]).days
            annual_return = (
                (1 + total_return / 100) ** (365 / days) - 1 if days > 0 else 0
            )
    ...
    def find_best_strategies(self):
        ...
        if daily_mo:
            ...
            logger.info(
                f"Daily Momentum: {daily_mo['annual_return']:.1f}% annual return"
            )
        ...
        if hourly_mr:
            ...
            hourly_mr["annual_return"] = (
                hourly_mr["annual_return"] * 1.5
        ...
        if min5_mr:
            ...
            min5_mr["annual_return"] = (
                min5_mr["total_return"] * 50
            )  # ~250 trading days / 5
            strategies.append(min5_mr)
            logger.info(
                f"5-min MR: {min5_mr['annual_return']:.1f}% projected annual return"
            )
