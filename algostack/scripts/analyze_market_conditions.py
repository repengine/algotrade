...
data["position_in_range"] = (data["close"] - data["low_20"]) / (
    data["high_20"] - data["low_20"]
)
...
print(
    f"  Total Return: {(data['close'].iloc[-1] / data['close'].iloc[0] - 1) * 100:.2f}%"
)
...
print(
    f"  Mean Reversion Setups: {mean_reversions} ({mean_reversions/len(data)*100:.1f}%)"
)
...
for hour, row in best_hours.iterrows():
    print(
        f"   {hour}:00 - Avg Move: {row['abs_mean']*100:.3f}%, Volatility: {row['std']*100:.3f}%"
    )
