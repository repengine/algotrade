    ...
    def backtest(self, data):
        ...
        for i in range(start_idx, len(df)):
            ...
            if position == 0:
                ...
                entry_signal = (
                    current_zscore < self.config['z_entry'] and
                    current_rsi < self.config['rsi_oversold'] and
                    current_volume_ratio > self.config['min_volume_ratio'] and
                    not df['is_last_hour'].iloc[i]  # Don't enter in last hour
                )
                
                if entry_signal:
                    # Calculate position size with leverage
                    buying_power = cash * self.config['leverage']
                    shares = int(buying_power * self.config['position_size'] / current_price)
            ...
            # EXIT LOGIC
                ...
                time_stop = (current_time - entry_time).total_seconds() > 7200  # 2 hour max hold
                ...
                if stop_loss_hit or target_hit or time_stop or market_close:
                    ...
                    net_cash = cash + (shares * current_price / self.config['leverage']) + gross_pnl
                    ...
                    trades[-1].update({
                        ...
                        'exit_reason': 'stop_loss' if stop_loss_hit else 
                                     ...
                                     'time_stop' if time_stop else 'market_close',
                        'pnl_pct': pnl_pct,
                        'gross_pnl': gross_pnl,
    ...
    def analyze_results(self, results, data):
        ...
        if len(trades) > 0:
            ...
            if pnls:
                ...
                print(f"Profit Factor: {abs(avg_win * len(winning_trades) / (avg_loss * len(losing_trades))):.2f}" 
                      if losing_trades and avg_loss != 0 else "N/A")
        ...
        if len(equity_curve) > 0:
            ...
            print(f"Maximum Drawdown (leveraged): {max_drawdown * self.config['leverage']:.2f}%")
    ...
    def save_configuration(self, results, filename_prefix="5min_volatility_crusher"):
        ...
        config_data = {
            ...
            "performance": {
                ...
                "projected_annual_return": results['total_return'] * 50  # Rough projection
            },
    ...
    if results['trades']:
        ...
        for trade in results['trades'][-5:]:
            if 'exit_time' in trade:
                ...
                print(f"  Z-Score: {trade['entry_zscore']:.2f}, RSI: {trade['entry_rsi']:.1f}")
                print(f"Exit: {trade['exit_time']} @ ${trade['exit_price']:.2f}")
                print(f"  Reason: {trade['exit_reason']}")
                print(f"  P&L: {trade['pnl_pct']:.2f}% | Hold Time: {trade.get('hold_time', 0):.1f} min")
