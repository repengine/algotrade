    ...
    def strategy_to_dashboard(signals: list[Any]) -> pd.DataFrame:
        ...
        for sig in signals:
            if hasattr(sig, "direction"):
                ...
                signal_data.append({"signal": sig, "strength": 1.0})
    ...
    if "talib" not in sys.modules:
        ...
        def create_dummy(name):
            def dummy(*args, **kwargs):
                warnings.warn(f"Indicator {name} not implemented, returning NaN", stacklevel=2)
                if args:
...
def validate_strategy_config(
    strategy_class: type, config: dict[str, Any]
) -> dict[str, Any]:
