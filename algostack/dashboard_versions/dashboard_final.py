...
def simulate_strategy_signals(strategy_name: str, data: pd.DataFrame, params: Dict[str, Any], 
    ...
    if "Mean Reversion" in strategy_name:
        ...
        exit_signal = ((rsi >= oversold) & (rsi <= overbought) & 
        ...
        st.text(f"RSI Strategy: {buy_signals} buy signals, {sell_signals} sell signals, {exit_signals} exit signals")
        ...
        st.text(f"MA Crossover Strategy: {long_signals} bullish crosses, {short_signals} bearish crosses")
    ...
    st.text(f"Total entries: {total_entries}, Total exits: {total_exits}, Final return: {final_return:.2f}%")
    
    # Show some sample trades
    if total_entries > 0:
        st.text("First few trades:")
        trade_points = signals[(signals['trade_entry'] == 1) | (signals['trade_exit'] == 1)]
        if len(trade_points) > 0:
            st.text(trade_points[['price', 'position', 'trade_type']].head(10).to_string())
...
def calculate_metrics(signals: pd.DataFrame) -> Dict[str, float]:
    ...
    if total_trades > 0:
        ...
        for i, (entry_idx, entry_row) in enumerate(entry_points.iterrows()):
            entry_price = entry_row['price']
            entry_position = entry_row['position']
            
            # Find the next trade point (exit or flip)
            future_points = signals[(signals.index > entry_idx) & 
