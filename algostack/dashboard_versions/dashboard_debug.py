    ...
    def _discover_strategies(self):
        ...
        for file_path in strategies_dir.glob("*.py"):
            ...
            try:
                ...
                add_debug_log(f"Failed to load strategy from {module_name}: {e}", "ERROR")
...
def fetch_data(symbol: str, start_date: datetime, end_date: datetime) -> pd.DataFrame:
    ...
    try:
        add_debug_log(f"Fetching data for {symbol} from {start_date} to {end_date}", "INFO")
...
def simulate_strategy_signals(strategy_name: str, data: pd.DataFrame, params: Dict[str, Any], 
    ...
    if len(data) < 50:
        add_debug_log(f"Not enough data for {strategy_name} (only {len(data)} days)", "WARNING")
        signals['returns'] = 0
    ...
    if "Mean Reversion" in strategy_name or "Hybrid" in strategy_name:
        ...
        signals.loc[buy_condition == True, 'signal'] = 1  # Buy
        signals.loc[sell_condition == True, 'signal'] = -1  # Sell
        
        add_debug_log(f"RSI strategy generated {(signals['signal'] != 0).sum()} signals", "DEBUG")
        ...
        signals.loc[long_condition == True, 'signal'] = 1
        signals.loc[short_condition == True, 'signal'] = -1
        
        add_debug_log(f"MA crossover strategy generated {(signals['signal'] != 0).sum()} signals", "DEBUG")
        ...
        signals.loc[buy_breakout == True, 'signal'] = 1
        signals.loc[sell_breakout == True, 'signal'] = -1
        
        add_debug_log(f"Breakout strategy generated {(signals['signal'] != 0).sum()} signals", "DEBUG")
        ...
        signals.loc[buy_momentum == True, 'signal'] = 1
        signals.loc[sell_momentum == True, 'signal'] = -1
        
        add_debug_log(f"Momentum strategy generated {(signals['signal'] != 0).sum()} signals", "DEBUG")
    ...
    position_diff = signals['position'].diff()
    ...
    add_debug_log(f"Trade entries: {entry_mask.sum()}, Trade exits: {exit_mask.sum()}", "INFO")
    ...
    if total_trades > 0:
        ...
        for entry_idx in entry_points.index:
            ...
            if len(future_exits) > 0:
                ...
                add_debug_log(f"Trade: Entry={entry_price:.2f}, Exit={exit_price:.2f}, Return={trade_return:.2%}", "DEBUG")
        ...
        if trade_results:
            ...
            add_debug_log(f"Completed trades: {total_trades}, Win rate: {win_rate:.1f}%", "INFO")
        else:
            win_rate = 0.0
...
def display_results(results: Dict):
    ...
    strategy_summary = df_metrics.groupby('strategy').agg({
        'total_return': 'mean',
        'sharpe_ratio': 'mean',
        'max_drawdown': 'mean',
        'win_rate': 'mean',
    ...
    for symbol, symbol_data in results.items():
        ...
        for i, (strategy_name, strategy_data) in enumerate(symbol_data['strategies'].items()):
            ...
            if 'cumulative_returns' in signals.columns:
                ...
                add_debug_log(f"{strategy_name} on {symbol}: Final return = {strategy_returns.iloc[-1]:.2f}%", "INFO")
            else:
                add_debug_log(f"No cumulative_returns column in {strategy_name} signals", "ERROR")
                ...
                for _, entry in entries.iterrows():
                    fig.add_trace(
                        go.Scatter(
                            ...
                            marker=dict(
                                symbol='triangle-up' if entry['position'] > 0 else 'triangle-down',
                                size=12,
                                color=color,
...
def main():
    ...
    st.markdown("### Integrated Strategy Testing & Analysis Platform with Enhanced Debugging")
