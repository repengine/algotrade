...
from algostack.backtests.run_backtests import BacktestEngine, run_walk_forward_optimization
...
class ComprehensiveBacktester:
    ...
    def load_api_keys(self):
        ...
        if config_path.exists():
            with open(config_path, 'r') as f:
                ...
                self.av_api_key = secrets.get('data_providers', {}).get('alphavantage', {}).get('api_key')
        else:
    ...
    def run_walk_forward_analysis(
        ...
        print(results_df[['window_start', 'window_end', 'total_return', 'sharpe_ratio', 'max_drawdown']])
        ...
        print(f"Consistency (Positive periods): {(results_df['total_return'] > 0).mean():.1%}")
    ...
    def _print_summary_comparison(self, results: list[Dict]):
        ...
        print(f"1. Highest Return: {df.iloc[df['total_return'].argmax()]['strategy']} ({df['total_return'].max():.1f}%)")
        print(f"2. Best Sharpe: {df.iloc[0]['strategy']} ({df.iloc[0]['sharpe_ratio']:.2f})")
        print(f"3. Lowest Drawdown: {df.iloc[df['max_drawdown'].argmin()]['strategy']} ({df['max_drawdown'].min():.1f}%)")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run comprehensive backtests for AlgoStack")
    parser.add_argument("--start", default="2020-01-01", help="Start date (YYYY-MM-DD)")
    parser.add_argument("--end", default="2023-12-31", help="End date (YYYY-MM-DD)")
    parser.add_argument("--capital", type=float, default=100000, help="Initial capital")
    parser.add_argument("--source", default="yfinance", choices=["yfinance", "alphavantage"], 
