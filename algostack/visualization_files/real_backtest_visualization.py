...
class VisualizedBacktester:
    ...
    def run_strategy_backtest(self, strategy, symbol, start_date, end_date):
        ...
        for i, (date, row) in enumerate(data.iterrows()):
            ...
            if len(portfolio_history) > 0:
                running_max = max([h['portfolio_value'] for h in portfolio_history] + [portfolio_value])
        ...
        self._create_visualizations(portfolio_df, trades_df, signals_df, data, symbol, metrics)
    ...
    def _calculate_metrics(self, portfolio_df, price_data):
        ...
        if len(aligned_price_data) > 0:
            benchmark_return = (aligned_price_data['close'].iloc[-1] / aligned_price_data['close'].iloc[0] - 1) * 100
        else:
            benchmark_return = 0
    ...
    def _create_visualizations(self, portfolio_df, trades_df, signals_df, price_data, symbol, metrics):
        ...
        aligned_price_data = price_data.loc[portfolio_df.index]
        benchmark_value = self.initial_capital * (aligned_price_data['close'] / aligned_price_data['close'].iloc[0])
        ...
        # Add benchmark
        ax1.plot(benchmark_value.index, benchmark_value, 
                label=f'{symbol} Buy & Hold', linewidth=2, color='gray', alpha=0.7)
        ...
        strategy_returns = (portfolio_df['portfolio_value'] / self.initial_capital - 1) * 100
        benchmark_returns = (benchmark_value / self.initial_capital - 1) * 100
        
        ax2.plot(portfolio_df.index, strategy_returns, 
                label='Strategy', linewidth=2, color='green')
        ax2.plot(benchmark_returns.index, benchmark_returns, 
                label=f'{symbol} Benchmark', linewidth=2, color='orange', alpha=0.7)
        ...
        ax3.plot(portfolio_df.index, portfolio_df['drawdown'] * 100, 
        ...
        ax4.plot(aligned_price_data.index, aligned_price_data['close'], 
                label=f'{symbol} Price', linewidth=1, color='black')
        ...
        if not signals_df.empty:
            ...
            if not sell_signals.empty:
                ax4.scatter(sell_signals['date'], sell_signals['price'], 
                           color='red', marker='v', s=100, label='Sell Signals', zorder=5)
        
        ax4.set_title(f'💹 {symbol} Price with Trading Signals', fontsize=14, fontweight='bold')
    ...
    if results:
        ...
        if not results['trades'].empty:
            ...
            for _, trade in recent_trades.iterrows():
                print(f"   {trade['date'].strftime('%Y-%m-%d')}: {trade['action']} {trade['shares']} shares @ ${trade['price']:.2f}")
    else:
