...
def enhanced_backtest(
    ...
    splitter = DataSplitter(
        method=split_config["method"], oos_ratio=split_config["oos_ratio"]
    )
    ...
    if validation_config["walk_forward"]:
        ...
        if param_ranges:  # Only run if we have parameters to optimize
            ...
            st.info(
                f"Running walk-forward analysis with {sum(1 for _ in [1 for v in param_ranges.values() for _ in range(len(v))])} parameter combinations..."
            )
            ...
            def run_wf():
                ...
                try:
                    wf_results = wf_analyzer.run_analysis(
                        strategy_class, data, param_ranges, wf_backtest_func, cost_model
                    )
            ...
            if thread.is_alive():
                st.error(
                    "Walk-forward analysis timed out after 5 minutes. Try reducing parameter combinations."
                )
            ...
            results["walk_forward"] = pd.DataFrame(
                {
                    ...
                    "sharpe_decay": [
                        calculate_performance_decay(is_results, oos_results)[
                            "sharpe_decay"
                        ]
                    ],
...
def add_transaction_costs(results, cost_model):
    ...
    results["transaction_costs"] = {
        ...
        "cost_percentage": total_costs
        / (results["initial_capital"] * results["total_return"] / 100),
    ...
    if results.get("sharpe_ratio", 0) > 0:
        results["net_sharpe"] = results["sharpe_ratio"] * (
            results["net_return"] / gross_return
        )
...
def calculate_performance_decay(is_results, oos_results):
    ...
    return {
        "sharpe_decay": decay,
        "return_decay": (
            is_results.get("total_return", 0) - oos_results.get("total_return", 0)
        )
...
def render_enhanced_results(results):
    ...
    if "regime_analysis" in results:
        ...
        if regime.get("regime_results") and len(regime["regime_results"]) > 0:
            ...
            if available_cols:
                st.dataframe(
                    regime_df[available_cols].round(2), use_container_width=True
                )
            else:
            ...
            st.warning(
                "Not enough data for regime analysis. Each regime needs at least 100 days of data."
            )
...
def main():
    st.set_page_config(
        page_title="AlgoStack Enhanced Dashboard", page_icon="📊", layout="wide"
    )

    st.title("📊 AlgoStack Trading Dashboard - Professional Edition")
    st.markdown(
        "*Enhanced with transaction costs, statistical validation, and walk-forward analysis*"
    )
    ...
    with st.sidebar.expander("Transaction Costs", expanded=True):
        ...
        slippage_model = st.selectbox(
            "Slippage Model", ["linear", "square_root"], help="Market impact model"
        )
    ...
    key_params = strategy_key_params.get(
        selected_strategy_name, ["lookback_period", "position_size", "stop_loss_pct"]
    )
        ...
        if param in default_params:
            ...
            if isinstance(param_value, bool):
                user_params[param] = st.sidebar.checkbox(
                    param.replace("_", " ").title(), value=param_value
                )
    ...
    if st.sidebar.button("🚀 Run Enhanced Backtest", type="primary"):
        ...
        with st.spinner("Running enhanced backtest..."):
            ...
            split_config = {"method": split_method, "oos_ratio": oos_ratio / 100}
