...
def trace_signal_generation():
    ...
    for i in range(60, min(len(strategy_data), 80)):  # Just check 20 days
        ...
        if day_of_week in ['Monday', 'Tuesday', 'Wednesday', 'Thursday']:
            ...
            print(f"    - Is holding day: {'Yes' if day_of_week in strategy.config.get('hold_days', []) else 'No'}")
            print(f"    - Positions < max: {'Yes' if len(strategy.positions) < strategy.config['max_positions'] else 'No'}")
            print(f"    - ATR in range: {'Yes' if latest['atr'] >= strategy.config['min_atr'] and latest['atr'] <= strategy.config['max_atr'] else 'No'}")
            print(f"    - Volume OK: {'Yes' if latest.get('volume_ratio', 1) >= strategy.config['volume_threshold'] else 'No'}")
