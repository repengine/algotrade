...
try:
    from strategy_defaults_fixed import get_strategy_defaults, merge_with_defaults, PARAMETER_TOOLTIPS
except ImportError:
...
def main():
    ...
    with st.sidebar:
        ...
        indicator_params = [p for p in default_params if any(ind in p for ind in ['period', 'threshold', 'band', 'zscore']) and p not in basic_params]
        risk_params = [p for p in default_params if any(risk in p for risk in ['stop', 'take', 'loss', 'profit']) and p not in basic_params + indicator_params]
        other_params = [p for p in default_params if p not in basic_params + indicator_params + risk_params and p != 'symbols']
