...
async def main():
    ...
    try:
        ...
        if not connected:
            logger.error(
                "Failed to connect. Please ensure gateway is running and you're authenticated."
            )
            return
        ...
        if contracts:
            ...
            await adapter.subscribe_market_data(
                apple_contract.conid, market_data_callback
            )
            ...
            hist_data = await adapter.get_historical_data(
                apple_contract.conid, period="1d", bar_size="5min"
            )
