...
Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"""
    ...
    def create_orders_table(self, orders: list) -> Table:
        """Create orders table."""
        table = Table(title="Active Orders", show_header=True, header_style="bold magenta")
        ...
        for order in orders[:10]:  # Show max 10 orders
            ...
            table.add_row(
                ...
                f"${order.get('limit_price', 0):.2f}" if order.get("limit_price") else "MKT",
                order.get("status", ""),
            )
    ...
    async def send_control():
        async with aiohttp.ClientSession() as session:
            try:
                ...
                async with session.post(f"{api_url}/api/system/control", json=data) as resp:
