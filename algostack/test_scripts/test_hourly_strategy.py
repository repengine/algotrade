...
for i in range(lookback_hours, len(hourly_data)):
    ...
    # Trading logic
    if position == 0 and zscore < -config['zscore_threshold'] and rsi < config['rsi_oversold']:
        # Buy signal
        ...
        if shares > 0:
            ...
            print(f"BUY at {current_time}: ${current_price:.2f} (z={zscore:.2f}, RSI={rsi:.1f})")
        ...
        print(f"SELL at {current_time}: ${current_price:.2f} (z={zscore:.2f}, RSI={rsi:.1f})")
        position = 0
        shares = 0
