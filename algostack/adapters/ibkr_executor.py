...
from algostack.adapters.ibkr_adapter import (
    ...
    OrderStatus as IBKROrderStatus,
...
class IBKRExecutor(BaseExecutor):
    ...
    async def submit_order(self, order: Order) -> str:
        ...
        try:
            ...
            if result and "order_id" in result:
                ...
                # Order rejected
                order.status = OrderStatus.REJECTED
                order.metadata["rejection_reason"] = result.get("error", "Unknown error") if result else "No response"
                self._notify_order_status(order)
                raise ValueError(f"Order rejected: {order.metadata['rejection_reason']}")
            ...
            for ibkr_pos in ibkr_positions:
                ...
                if symbol:
                    positions[symbol] = Position(
                        ...
                        current_price=ibkr_pos.market_value / ibkr_pos.position if ibkr_pos.position != 0 else 0,
    ...
    def _convert_order(self, order: Order, contract: IBKRContract) -> IBKROrder:
        ...
        # Map side
        ibkr_side = IBKROrderSide.BUY if order.side == OrderSide.BUY else IBKROrderSide.SELL
                ...
                if our_order_id and our_order_id in self._orders:
                    ...
                    if "lastFillTime" in update and update.get("filledQuantity", 0) > 0:
                        ...
                        fill = Fill(
                            ...
                            symbol=order.symbol,
                            side=order.side,
                            quantity=int(update.get("lastFillQuantity", order.filled_quantity)),
                            price=float(update.get("lastFillPrice", order.average_fill_price)),
