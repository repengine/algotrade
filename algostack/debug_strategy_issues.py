        ...
        if 'error' in results:
            ...
            if not signals_df.empty:
                ...
                if 'direction' in signals_df.columns:
                    print(f"Signal breakdown: {signals_df['direction'].value_counts().to_dict()}")
...
def check_data_differences():
    ...
    if test_date in av_data.index.strftime('%Y-%m-%d'):
        ...
        print(f"  Alpha Vantage: close={av_row['close']:.2f}, volume={av_row['volume']:.0f}")
    ...
    if test_date in yf_data.index.strftime('%Y-%m-%d'):
        ...
        print(f"  Yahoo Finance: close={yf_row[close_col]:.2f}, volume={yf_row[vol_col]:.0f}")
