...
class MonitoringAPI:
    ...
    def _setup_routes(self):
        ...
        # System endpoints
        self.app.get("/api/system/info", response_model=SystemInfo)(self.get_system_info)
        self.app.post("/api/system/control")(self.control_system)
        
        # Strategy endpoints
        self.app.get("/api/strategies", response_model=List[StrategyInfo])(self.get_strategies)
        self.app.get("/api/strategies/{strategy_id}", response_model=StrategyInfo)(self.get_strategy)
        self.app.post("/api/strategies/control")(self.control_strategy)
        
        # Position endpoints
        self.app.get("/api/positions", response_model=List[PositionInfo])(self.get_positions)
        self.app.get("/api/positions/{symbol}", response_model=PositionInfo)(self.get_position)
        ...
        self.app.get("/api/performance", response_model=PerformanceMetrics)(self.get_performance)
    ...
    async def get_system_info(self) -> SystemInfo:
        ...
        if self.engine.stats["engine_start"]:
            uptime = (datetime.now() - self.engine.stats["engine_start"]).total_seconds()
            
        return SystemInfo(
            status=SystemStatus.RUNNING if self.engine.is_running else SystemStatus.OFFLINE,
    ...
    async def get_strategies(self) -> list[StrategyInfo]:
        ...
        for strategy_id, strategy in self.engine.strategies.items():
            info = StrategyInfo(
                ...
                orders_placed=len(self.engine.order_manager.get_strategy_orders(strategy_id)),
    ...
    async def get_positions(self) -> list[PositionInfo]:
        ...
        try:
            ...
            for symbol, pos in positions.items():
                pnl_pct = (pos.unrealized_pnl / (pos.average_cost * pos.quantity) * 100 
    ...
    async def create_order(self, command: OrderCommand):
        ...
        try:
            ...
            from algostack.core.executor import OrderSide, OrderType, TimeInForce
    ...
    async def get_performance(self) -> PerformanceMetrics:
        ...
        # Calculate daily P&L
        today_trades = [
            t for t in metrics_collector.trades
            if t.exit_time.date() == datetime.now().date()
        ]
        daily_pnl = sum(t.pnl for t in today_trades)
        daily_pnl_pct = (daily_pnl / portfolio.initial_capital) * 100 if portfolio.initial_capital > 0 else 0
        ...
        return PerformanceMetrics(
            ...
            total_pnl_percentage=(perf_metrics.get("total_pnl", 0) / portfolio.initial_capital * 100),
        ...
        while True:
            try:
                ...
                await self.manager.broadcast(
                    WSMessage(type="positions", data=positions),
                    "positions"
                )
                ...
                await self.manager.broadcast(
                    WSMessage(type="orders", data=orders),
                    "orders"
                )
                ...
                await self.manager.broadcast(
                    WSMessage(type="metrics", data=metrics),
                    "metrics"
                )
