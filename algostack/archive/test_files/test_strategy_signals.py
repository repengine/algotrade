...
def test_all_strategies():
    ...
    for strategy_name, strategy_class in manager.strategies.items():
        ...
        if strategy_name == "IntradayORB":
            ...
            intraday_data = data_manager.fetch_data(
                "SPY", "1mo", "15m", "alpha_vantage"
            )
        ...
        user_params = {"symbol": "SPY", "position_size": 0.95}
        ...
        if default_lookback >= data_length * 0.8:
            ...
            print(
                f"  Adjusted lookback from {default_lookback} to {user_params['lookback_period']}"
            )
