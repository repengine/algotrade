...
def compare_indicators():
    ...
    if len(common_dates) > 0:
        ...
        for date in compare_dates:
            ...
            print(f"    Close: {av_row['close']:.2f}, RSI: {av_row['rsi']:.2f}, Lower Band: {av_row['lower_band']:.2f}")
            print(f"    Volume Ratio: {av_row['volume_ratio']:.2f}")
            print(f"  Yahoo Finance:")
            print(f"    Close: {yf_row['close']:.2f}, RSI: {yf_row['rsi']:.2f}, Lower Band: {yf_row['lower_band']:.2f}")
    ...
    av_entries = av_indicators[(av_indicators['rsi'] < 40) & (av_indicators['close'] < av_indicators['lower_band'])]
    yf_entries = yf_indicators[(yf_indicators['rsi'] < 40) & (yf_indicators['close'] < yf_indicators['lower_band'])]
