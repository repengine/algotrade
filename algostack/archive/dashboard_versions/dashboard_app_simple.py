...
def fetch_data(symbol: str, start_date: datetime, end_date: datetime) -> pd.DataFrame:
    ...
    try:
        ...
        if data.empty:
            ...
            st.success(f"✓ Fetched {len(data)} days of data for {symbol} ({actual_start} to {actual_end})")
...
def calculate_simple_metrics(data: pd.DataFrame, initial_capital: float = 100000) -> Dict[str, float]:
    ...
    days = len(data)
    if days > 0:
        annual_return = ((1 + total_return/100) ** (252/days) - 1) * 100 if days < 252 else total_return
    else:
        annual_return = 0.0
...
def run_simple_backtest(symbols: List[str], start_date: datetime, end_date: datetime, 
...
def main():
    ...
    with st.sidebar:
        ...
        if strategies:
            st.info(f"Found {len(strategies)} strategies:\n" + "\n".join(f"• {s}" for s in strategies[:5]))
            if len(strategies) > 5:
