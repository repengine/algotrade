#!/usr/bin/env python3
"""
AlgoStack Dashboard - Final Working Version
"""
...
def simulate_strategy_signals(
    ...
    if "Mean Reversion" in strategy_name:
        ...
        # Track RSI for debugging
        signals["rsi"] = rsi

        # Generate signals only when RSI crosses thresholds
        ...
        exit_signal = (
            (rsi >= oversold)
            & (rsi <= overbought)
            & ((rsi.shift(1) < oversold) | (rsi.shift(1) > overbought))
        )
        ...
        st.text(
            f"RSI Strategy: {buy_signals} buy signals, {sell_signals} sell signals, {exit_signals} exit signals"
        )
        ...
        # Debug info
        long_signals = bullish_cross.sum()
        short_signals = bearish_cross.sum()
        st.text(
            f"MA Crossover Strategy: {long_signals} bullish crosses, {short_signals} bearish crosses"
        )
    ...
    # Entry: position changes from 0 to non-zero OR position flips
    entry_mask = (
        (signals["position"] != 0) & (signals["position"].shift(1).fillna(0) == 0)
    ...
    # Exit: position changes to 0 from non-zero
    exit_mask = (signals["position"] == 0) & (
        signals["position"].shift(1).fillna(0) != 0
    )
    signals.loc[exit_mask, "trade_exit"] = 1
    signals.loc[exit_mask, "trade_type"] = "EXIT"

    # Also mark exits when position flips (these are also entries)
    flip_mask = (signals["position"] * signals["position"].shift(1).fillna(0)) < 0
    signals.loc[flip_mask & (signals["position"].shift(1) != 0), "trade_exit"] = 1
    ...
    st.text(
        f"Total entries: {total_entries}, Total exits: {total_exits}, Final return: {final_return:.2f}%"
    )

    # Show some sample trades
    if total_entries > 0:
        st.text("First few trades:")
        trade_points = signals[
            (signals["trade_entry"] == 1) | (signals["trade_exit"] == 1)
        ]
        if len(trade_points) > 0:
            st.text(
                trade_points[["price", "position", "trade_type"]].head(10).to_string()
            )
...
def calculate_metrics(signals: pd.DataFrame) -> dict[str, float]:
    ...
    if total_trades > 0:
        ...
        for _i, (entry_idx, entry_row) in enumerate(entry_points.iterrows()):
            entry_price = entry_row["price"]
            entry_position = entry_row["position"]

            # Find the next trade point (exit or flip)
            future_points = signals[
                (signals.index > entry_idx)
                & ((signals["trade_exit"] == 1) | (signals["trade_entry"] == 1))
            ]

            if len(future_points) > 0:
                exit_idx = future_points.index[0]
...
def main():
    ...
    if st.checkbox("Show Debug Info", value=False):
        ...
        st.write(
            "Number of enabled strategies:", len(st.session_state.enabled_strategies)
        )
    ...
    with st.sidebar:
        ...
        symbols = st.multiselect("Select Symbols", default_symbols, default=["SPY"])
