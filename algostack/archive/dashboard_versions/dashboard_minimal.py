...
# Page configuration
st.set_page_config(
    page_title="AlgoStack Dashboard (Minimal)", page_icon="📈", layout="wide"
)
...
def ma_crossover_strategy(
    data: pd.DataFrame, fast: int = 10, slow: int = 30
) -> pd.DataFrame:
...
def rsi_strategy(
    data: pd.DataFrame, period: int = 14, oversold: int = 30, overbought: int = 70
) -> pd.DataFrame:
        ...
        if not entries.empty:
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="markers",
                    name=f"{name} Buy",
                    marker={"symbol": "triangle-up", "size": 10, "color": color},
                    showlegend=False,
                ),
                row=1,
                col=1,
    ...
    if run_btn:
        ...
        if use_rsi:
            with st.spinner("Running RSI strategy..."):
                rsi_signals = rsi_strategy(
                    data, rsi_period, rsi_oversold, rsi_overbought
                )
        ...
        if strategies:
            ...
            for name, signals in strategies.items():
                ...
                if not trades.empty:
                    ...
                    trades = trades[["price", "Action"]].rename(
                        columns={"price": "Price"}
                    )
