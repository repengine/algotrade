            ...
            with col:
                ...
                if isinstance(default_value, bool):
                    configured_params[param_name] = st.checkbox(
                        param_name.replace("_", " ").title(), value=default_value
                    )
...
def aggregate_backtest_results(all_results: dict[str, dict]) -> dict:
    ...
    for _result_key, result in all_results.items():
        ...
        strategy_metrics[strategy_name]["total_return"] += metrics.get(
            "total_return", 0
        )
        strategy_metrics[strategy_name]["total_trades"] += metrics.get(
            "total_trades", 0
        )
        strategy_metrics[strategy_name]["winning_trades"] += metrics.get(
            "winning_trades", 0
        )
        strategy_metrics[strategy_name]["losing_trades"] += metrics.get(
            "losing_trades", 0
        )
    ...
    win_rate = (
        total_winners / (total_winners + total_losers)
    ...
    aggregated = {
        "metrics": {
            ...
            "sharpe_ratio": np.mean(
                [r["metrics"].get("sharpe_ratio", 0) for r in all_results.values()]
            ),
            "max_drawdown": np.max(
                [r["metrics"].get("max_drawdown", 0) for r in all_results.values()]
            ),
            "sortino_ratio": np.mean(
                [r["metrics"].get("sortino_ratio", 0) for r in all_results.values()]
            ),
            "calmar_ratio": np.mean(
                [r["metrics"].get("calmar_ratio", 0) for r in all_results.values()]
            ),
            "profit_factor": np.mean(
                [r["metrics"].get("profit_factor", 0) for r in all_results.values()]
            ),
        },
        "trades": all_trades,
        "signals": all_signals,
...
def run_backtest(
    ...
    engine = BacktestEngine(data_handler=data_handler, initial_capital=initial_cash)
    ...
    for strategy_name, params in strategies.items():
        ...
        if strategy_class:
            try:
                ...
                strategy_config = {"name": strategy_name, "symbols": symbols, **params}
                ...
                for symbol in symbols:
                    ...
                    all_results[result_key] = {
                        ...
                        "symbol": symbol,
                        "metrics": metrics,
                        "signals": engine.results.get(strategy.name, {}).get(
                            "signals", []
                        ),
                        "trades": engine.results.get(strategy.name, {}).get(
                            "trades", []
                        ),
                    }
...
def display_backtest_results(results: dict):
    ...
    if "trades" in results and results["trades"]:
            ...
            fig = go.Figure(
                data=[
                    go.Bar(
                        ...
                        y=[
                            len(winners),
                            winners["pnl"].mean() if len(winners) > 0 else 0,
                        ],
                    ),
                    go.Bar(
                        name="Losers",
        ...
        st.dataframe(
            trades_df.tail(20)[
                ["datetime", "symbol", "side", "quantity", "price", "pnl"]
            ],
    ...
    if "strategy_metrics" in results:
        ...
        if "individual_results" in results:
            ...
            for _result_key, result in results["individual_results"].items():
...
def main():
    ...
    with st.sidebar:
        ...
        symbols = st.multiselect(
            "Select Symbols", default_symbols, default=["AAPL", "MSFT", "SPY"]
        )
        ...
        with col1:
            if st.button("📊 Export Results (CSV)"):
                ...
                if "trades" in st.session_state.backtest_results:
                    ...
                    st.download_button(
                        "Download Trades CSV", csv, "backtest_trades.csv", "text/csv"
                    )
        ...
        with col2:
            if st.button("📈 Export Metrics (JSON)"):
                ...
                metrics_json = json.dumps(
                    st.session_state.backtest_results.get("metrics", {}), indent=2
                )
                st.download_button(
