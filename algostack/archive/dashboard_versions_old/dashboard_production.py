#!/usr/bin/env python3
"""
AlgoStack Dashboard - Production Version (Fixed)

This version includes fixes for:
1. RSI function signature issues
2. HybridRegime parameter validation
...
from strategy_validation_bypass import patch_all_validators

patch_all_validators()

# Import integration helpers with fixes
try:
    from strategy_integration_helpers_fixed import (
        DataFormatConverter,
        RiskContextMock,
        TechnicalIndicators,
        patch_talib_imports,
...
# Import strategy defaults with fixes
try:
    from strategy_defaults_fixed import (
        PARAMETER_TOOLTIPS,
        get_strategy_defaults,
        merge_with_defaults,
    )
except ImportError:
    from strategy_defaults import get_strategy_defaults, merge_with_defaults
    ...
    def run_backtest(
        ...
        try:
            ...
            strategy_data = self.converter.dashboard_to_strategy(
                data, symbol=user_params.get("symbol", "UNKNOWN")
            )
...
def calculate_metrics(
    results: dict[str, Any], initial_capital: float = 100000
) -> dict[str, float]:
    ...
    sharpe = (
        np.sqrt(252) * excess_returns.mean() / returns.std() if returns.std() > 0 else 0
    )
    ...
    num_trades = len(
        [t for t in trades if t["action"] in ["BUY", "SELL", "EXIT", "CLOSE"]]
    )
...
def create_performance_chart(
    data: pd.DataFrame, results: dict[str, Any], initial_capital: float, symbol: str
) -> go.Figure:
    ...
    # Add sell signals
    sell_trades = [
        t for t in results["trades"] if t["action"] in ["SELL", "EXIT", "CLOSE"]
    ]
    if sell_trades:
...
def main():
    """Main Streamlit app."""
    st.set_page_config(
        page_title="AlgoStack Trading Dashboard", page_icon="📊", layout="wide"
    )

    st.title("📊 AlgoStack Trading Dashboard - Production")
    ...
    with st.sidebar:
        ...
        with col2:
            end_date = st.date_input(
                "End Date", value=datetime.now(), help="End of backtest period"
            )
        ...
        # Group parameters by category - avoid duplicates
        basic_params = ["lookback_period", "position_size", "max_positions"]
        indicator_params = [
            p
            for p in default_params
            if any(ind in p for ind in ["period", "threshold", "band", "zscore"])
            and p not in basic_params
        ]
        risk_params = [
            p
            for p in default_params
            if any(risk in p for risk in ["stop", "take", "loss", "profit"])
            and p not in basic_params + indicator_params
        ]
        other_params = [
            p
            for p in default_params
            if p not in basic_params + indicator_params + risk_params and p != "symbols"
        ]

        # Track which parameters we've already displayed
        displayed_params = set()

        # Display parameters by category
        if any(p in default_params for p in basic_params):
            st.markdown("**Basic Parameters**")
            for param in basic_params:
                if param in default_params and param not in displayed_params:
                    ...
                    if isinstance(default_params[param], float):
                        user_params[param] = st.number_input(
                            ...
                            value=float(default_params[param]),
                            format="%.4f",
                            help=tooltip,
                            key=f"{selected_strategy_name}_{param}_basic",
                        )
                    elif isinstance(default_params[param], int):
                        user_params[param] = st.number_input(
                            param.replace("_", " ").title(),
                            value=int(default_params[param]),
                            step=1,
                            help=tooltip,
                            key=f"{selected_strategy_name}_{param}_basic",
                        )

        if any(p in default_params for p in indicator_params):
                ...
                if param in default_params and param not in displayed_params:
                    ...
                    if isinstance(default_params[param], float):
                        user_params[param] = st.number_input(
                            ...
                            format="%.4f",
                            help=tooltip,
                            key=f"{selected_strategy_name}_{param}_ind",
                        )
                    elif isinstance(default_params[param], int):
                        user_params[param] = st.number_input(
                            param.replace("_", " ").title(),
        ...
        # Risk parameters
        if any(p in default_params for p in risk_params):
                ...
                if param in default_params and param not in displayed_params:
                    ...
                    if isinstance(default_params[param], float):
                        user_params[param] = st.number_input(
                            ...
                            format="%.4f",
                            help=tooltip,
                            key=f"{selected_strategy_name}_{param}_risk",
                        )
                    elif isinstance(default_params[param], int):
                        user_params[param] = st.number_input(
                            param.replace("_", " ").title(),
        ...
        if other_params:
            ...
            for param in other_params:
                if param not in displayed_params:
                    displayed_params.add(param)
                    tooltip = PARAMETER_TOOLTIPS.get(param, f"Parameter: {param}")
                    if isinstance(default_params[param], (int, float)):
                        if isinstance(default_params[param], float):
                            user_params[param] = st.number_input(
                                param.replace("_", " ").title(),
                                value=float(default_params[param]),
                                format="%.4f",
                                help=tooltip,
                                key=f"{selected_strategy_name}_{param}_other",
                            )
                        else:
                            user_params[param] = st.number_input(
                                param.replace("_", " ").title(),
                        ...
                        user_params[param] = st.checkbox(
                            param.replace("_", " ").title(),
                            value=bool(default_params[param]),
                            help=tooltip,
                            key=f"{selected_strategy_name}_{param}_other",
                        )
                    ...
                    elif isinstance(default_params[param], list):
                        # Skip lists for now (like volatility_bands)
                        user_params[param] = default_params[param]

        # Initial capital
        initial_capital = st.number_input(
            "Initial Capital", value=100000, step=10000, help="Starting portfolio value"
        )
    ...
    if run_backtest:
        ...
        with st.spinner(
            f"Running {manager.get_display_name(selected_strategy_name)} backtest..."
        ):
            # Run backtest
