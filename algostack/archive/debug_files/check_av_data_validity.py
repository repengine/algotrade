...
def check_data_validity():
    ...
    subset_valid = (
        (subset["high"] >= subset["low"]).all() and
        (subset["high"] >= subset["open"]).all() and
        (subset["high"] >= subset["close"]).all() and
        (subset["low"] <= subset["open"]).all() and
        (subset["low"] <= subset["close"]).all()
    )
