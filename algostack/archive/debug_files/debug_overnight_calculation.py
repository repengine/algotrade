    ...
    for data_source in ["alpha_vantage", "yfinance"]:
        ...
        for i in range(1, len(strategy_data)):
            ...
            if strategy_data["close"].iloc[i - 1] > 0:
                overnight_ret = (
                    strategy_data["open"].iloc[i] / strategy_data["close"].iloc[i - 1]
                ) - 1
        ...
        print(
            f"Average overnight return: {overnight_df['overnight_return'].mean():.6f}"
        )
        print(
            f"Annualized overnight return: {overnight_df['overnight_return'].mean() * 252:.4f}"
        )

        # By day of week
        print("\nOvernight returns by day of week:")
        for day in ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"]:
            day_returns = overnight_df[overnight_df["day_of_week"] == day][
                "overnight_return"
            ]
        ...
        for _, row in top_returns.iterrows():
            print(
                f"  {row['date'].strftime('%Y-%m-%d')} ({row['day_of_week']}): {row['overnight_return']:.4f}"
            )
