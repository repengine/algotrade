...
def test_timezone_fix():
    ...
    results = strategy_manager.run_backtest(
        strategy_class, strategy_name, user_params, av_data_tz, initial_capital=100000
    )
    ...
    if "error" in results:
        ...
        if not signals_df.empty:
            ...
            for idx in test_data.index[-5:]:
                ...
                if day_of_week in strategy.config.get(
                    "hold_days", ["Monday", "Tuesday", "Wednesday", "Thursday"]
                ):
                    print("    ✓ Is a holding day")
                else:
                    print("    ✗ Not a holding day")
