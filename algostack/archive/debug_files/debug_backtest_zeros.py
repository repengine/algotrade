    ...
    df = pd.DataFrame(
        {
            ...
            "volume": [
                1000000 + np.random.randint(-100000, 100000) for _ in range(days)
            ],
        }
    )
...
def test_strategy_directly(data, symbol="TEST"):
    ...
    logger.info(
        f"Processing {len(data) - lookback} bars starting from index {lookback}"
    )
    ...
    for i in range(lookback, len(data)):
        try:
            ...
            if signal is not None:
                ...
                logger.info(
                    f"Signal at {data.index[i]}: {signal.direction} with strength {signal.strength}"
                )
...
def test_with_synthetic_data():
    ...
    if signals is not None:
        ...
        for i in range(len(signals)):
            ...
            logger.debug(
                f"Processing date {date}: signal={signal_row.get('signal', 0)}, price={price_row['close']}"
            )

            # Track returns
            if i > 0:
                ret = (price_row["close"] - data.iloc[i - 1]["close"]) / data.iloc[
                    i - 1
                ]["close"]
                returns.append(ret)
        ...
        logger.info(
            f"Returns statistics: mean={np.mean(returns):.4f}, std={np.std(returns):.4f}"
        )
...
def test_with_yfinance():
    ...
    try:
        ...
        # Test strategy directly
        test_strategy_directly(data, symbol)
...
def test_full_backtest_pipeline():
    ...
    for i in range(config["lookback_period"], len(data)):  # Start after lookback period
        ...
        if signal is not None:
            signals_generated += 1
            logger.info(
                f"Signal at {timestamp}: {signal.direction} @ ${current_price:.2f}"
            )
            ...
            if signal.symbol not in positions and signal.direction in ["LONG", "SHORT"]:
                ...
                if shares > 0:
                    ...
                    orders_placed += 1
                    logger.info(
                        f"  Opened {signal.direction} position: {shares} shares @ ${current_price:.2f}"
                    )
        ...
        for i in range(20, len(data)):  # Start after enough data for indicators
            if not np.isnan(rsi.iloc[i]) and not np.isnan(lower_band.iloc[i]):
                ...
                if rsi.iloc[i] < 10 and data["close"].iloc[i] < lower_band.iloc[i]:
                    ...
                    logger.info(
                        f"  Entry condition met at index {i}: RSI={rsi.iloc[i]:.2f}, Close={data['close'].iloc[i]:.2f}, Lower Band={lower_band.iloc[i]:.2f}"
                    )
...
def check_data_format_compatibility():
    ...
    for i, columns in enumerate(test_formats):
        ...
        data = pd.DataFrame(
            {col: np.random.randn(50) + 100 for col in columns}, index=dates
        )
