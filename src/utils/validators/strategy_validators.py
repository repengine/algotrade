...
class StrategyParameterValidator:
    ...
    def validate_config(
        ...
        optional_params: Optional[dict[str, tuple]] = None,
    ) -> dict[str, Any]:
        """
...
def validate_mean_reversion_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    validator.validate_numeric_range(
        validated["exit_zscore"], 0, validated["zscore_threshold"], "exit_zscore"
    )
    ...
    validator.validate_numeric_range(
        validated["rsi_overbought"], 0, 100, "rsi_overbought"
    )
...
def validate_trend_following_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    validator.validate_numeric_range(
        validated["adx_threshold"], 0, 100, "adx_threshold"
    )
...
def validate_pairs_trading_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    validator.validate_numeric_range(
        validated["zscore_exit"], 0, validated["zscore_entry"], "zscore_exit"
    )
    validator.validate_positive(validated["lookback_window"], "lookback_window")
    validator.validate_percentage(
        validated["correlation_threshold"], "correlation_threshold"
    )
...
def validate_intraday_orb_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    validator.validate_positive(
        validated["opening_range_minutes"], "opening_range_minutes"
    )
