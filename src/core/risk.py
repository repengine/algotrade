...
class EnhancedRiskManager:
        ...
        **kwargs,
        ...
        if config is None and risk_limits is not None:
            ...
            if hasattr(risk_limits, "__dict__"):
                ...
                config["max_portfolio_volatility"] = getattr(
                    rl,
                    "max_portfolio_volatility",
                    getattr(rl, "max_portfolio_risk", 0.10),
                )
                config["max_var_95"] = getattr(
                    rl, "max_var_95", getattr(rl, "max_single_loss", 0.02)
                )
                config["max_drawdown"] = getattr(
                    rl, "max_drawdown", getattr(rl, "max_daily_loss", 0.15)
                )
                config["max_correlation"] = getattr(rl, "max_correlation", 0.70)
                config["max_sector_exposure"] = getattr(
                    rl, "max_sector_exposure", getattr(rl, "max_concentration", 0.40)
                )
                config["concentration_limit"] = getattr(
                    rl, "concentration_limit", getattr(rl, "max_concentration", 0.60)
                )
        ...
        self.portfolio_value = kwargs.get(
            "portfolio_value", 100000
        )  # Backward compatibility
        self.volatility_lookback = kwargs.get(
        ...
        if risk_limits is not None:
            ...
            self.risk_limits = {
                ...
                "max_portfolio_volatility": config.get(
        ...
        self.metrics_history_limit = config.get(
            "metrics_history_limit", 252
        )  # Default to one trading year
    ...
    def calculate_risk_metrics(
        ...
        current_drawdown = float(
            abs(drawdown.iloc[-1]) if not drawdown.empty else 0.0
    ...
    def _calculate_beta(
        self, daily_returns: pd.Series, benchmark_returns: Optional[pd.Series]
    ) -> float:
        """Calculate portfolio beta against benchmark with robust error handling."""
        if (
            benchmark_returns is None
            or len(benchmark_returns) == 0
            or len(daily_returns) == 0
        ):
            return 1.0
        ...
        try:
            ...
            common_index = daily_returns.index.intersection(
                list(aligned_benchmark.index)
            )
            ...
            if (
                pd.isna(covariance_result)
                or pd.isna(variance_result)
                or variance_result == 0
            ):
                return 1.0
    ...
    def check_risk_compliance(
        ...
        if portfolio and hasattr(portfolio, "get_returns_history"):
            ...
            if not returns.empty:  # Add check for empty returns
                ...
                logger.warning(
                    "Portfolio returns history is empty, cannot calculate risk metrics."
                )
                self.risk_metrics = self._default_risk_metrics()
        elif not self.risk_metrics:  # If not calculated and not already set
            logger.warning(
                "No portfolio data to calculate risk metrics, using defaults."
            )
            self.risk_metrics = self._default_risk_metrics()
        ...
        if isinstance(self.risk_limits, dict):
            ...
            max_var_95 = getattr(self.risk_limits, "max_var_95", None) or getattr(
                self.risk_limits, "max_single_loss", 0.02
            )
            max_portfolio_vol = getattr(
                self.risk_limits, "max_portfolio_volatility", None
            ) or getattr(self.risk_limits, "max_portfolio_risk", 0.10)
            max_drawdown = getattr(self.risk_limits, "max_drawdown", None) or getattr(
                self.risk_limits, "max_daily_loss", 0.15
            )
            min_sharpe = getattr(self.risk_limits, "min_sharpe", None) or 0.5
            concentration_limit = getattr(
                self.risk_limits, "concentration_limit", None
            ) or getattr(self.risk_limits, "max_concentration", 0.60)
            max_sector_exposure = getattr(
                self.risk_limits, "max_sector_exposure", None
            ) or getattr(self.risk_limits, "max_concentration", 0.40)

        # Check VaR limit
        if self.risk_metrics and abs(self.risk_metrics.value_at_risk) > max_var_95:
            violations.append(
        ...
        if (
            self.risk_metrics
            and self.risk_metrics.portfolio_volatility > max_portfolio_vol
        ):
            violations.append(
        ...
        # Check drawdown
        if self.risk_metrics and abs(self.risk_metrics.current_drawdown) > max_drawdown:
            violations.append(
        ...
        if positions and portfolio_value:
            ...
            for _symbol, position_data in positions.items():
                ...
                sector_exposure[sector] = (
                    sector_exposure.get(sector, 0) + position_value
                )
    ...
    def get_risk_adjusted_sizes(
        ...
        for signal in signals:
            ...
            # Get risk limits
            if isinstance(self.risk_limits, dict):
                max_portfolio_vol = self.risk_limits.get(
            ...
            vol_scaled_size = min(vol_scaled_size, max_position)
    ...
    def update_risk_state(self, portfolio: Any) -> None:
        ...
        if self.risk_metrics:
            ...
            if abs(self.risk_metrics.current_drawdown) > max_drawdown * 0.8:
                self.is_risk_on = False
                logger.warning("Entering risk-off mode due to drawdown")
            elif abs(self.risk_metrics.current_drawdown) < max_drawdown * 0.5:
                self.is_risk_on = True
    ...
    def get_risk_report(self) -> dict[str, Any]:
        ...
        report: dict[str, Any] = {
            ...
            "violations": [],
            "historical_metrics": self.historical_metrics[-10:]
    ...
    def update_historical_metrics(self, metrics: RiskMetrics) -> None:
        ...
        if len(self.historical_metrics) > self.metrics_history_limit:
            self.historical_metrics = self.historical_metrics[
    ...
    def get_average_metrics(
        ...
        avg_vol = sum(m.portfolio_volatility for m in recent_metrics) / len(
            recent_metrics
        )
    ...
    def can_trade(self, strategy: Any) -> bool:
        ...
        max_drawdown = self._get_risk_limit("max_drawdown", 0.15)
        if self.risk_metrics and self.risk_metrics.current_drawdown > max_drawdown:
            logger.warning(
    ...
    def check_order(self, order: Any, portfolio: Any) -> bool:
        ...
        if side_str == "buy":
            required_cash = position_value * (
                1 + self.config.get("margin_buffer", 0.25)
            )
        ...
        if hasattr(portfolio, "daily_pnl") and portfolio.daily_pnl:
            ...
            daily_loss_pct = (
                abs(daily_loss) / portfolio.current_equity
                if portfolio.current_equity > 0
                else 0
            )
    ...
    def validate_pre_trade_risk(self, order: Any, portfolio: Any) -> dict[str, Any]:
        ...
        """
        result = {"approved": True, "violations": [], "warnings": []}
        ...
        if (
            self.risk_metrics
            and self.risk_metrics.value_at_risk * portfolio.current_equity > max_var
        ):
            result["violations"].append(
    ...
    def check_position_size(
        ...
        if symbol in position_limits:
            ...
            max_size = (
                self.risk_limits.get("max_position_size", 0.20)
                if isinstance(self.risk_limits, dict)
                else getattr(self.risk_limits, "max_position_size", 0.20)
            )
    ...
    def calculate_portfolio_risk(
        self, positions: dict, market_volatilities: dict
    ) -> float:
        """Calculate portfolio risk based on positions and volatilities."""
        total_risk = 0.0
        total_value = sum(
            pos.get("value", pos.get("market_value", 0)) for pos in positions.values()
        )
    ...
    def check_portfolio_risk(self, portfolio_risk: float) -> bool:
        ...
        max_risk = (
            self.risk_limits.get("max_portfolio_risk", 0.06)
            if isinstance(self.risk_limits, dict)
            else getattr(self.risk_limits, "max_portfolio_risk", 0.06)
        )
    ...
    def check_daily_loss(self, daily_pnl: float, portfolio_value: float) -> bool:
        ...
        max_loss = (
            self.risk_limits.get("max_daily_loss", 0.03)
            if isinstance(self.risk_limits, dict)
            else getattr(self.risk_limits, "max_daily_loss", 0.03)
        )
    ...
    def check_leverage(self, total_exposure: float, portfolio_value: float) -> bool:
        ...
        max_leverage = (
            self.risk_limits.get("max_leverage", 1.5)
            if isinstance(self.risk_limits, dict)
            else getattr(self.risk_limits, "max_leverage", 1.5)
        )
    ...
    def calculate_correlation_risk(
        self, correlation_matrix: pd.DataFrame, positions: list, weights: np.ndarray
    ) -> float:
        ...
        if len(available_positions) < 2:
            logger.warning(
                f"Insufficient positions in correlation matrix: {available_positions}"
            )
            return 0.0
        ...
        if avg_correlation > 0.7:
            logger.warning(
                f"HIGH CORRELATION RISK: {avg_correlation:.2f} - Portfolio at risk of simultaneous losses!"
            )
    ...
    def assess_liquidity(
        ...
        for symbol, position_size in position_sizes.items():
            ...
            if volume_percentage >= impact_threshold:
                ...
                score = 0.0
                logger.warning(
                    f"LIQUIDITY RISK: {symbol} position is {volume_percentage:.1%} of daily volume!"
                )
            else:
    ...
    def update_sector_allocations(self, portfolio_state: dict[str, Any]) -> None:
        ...
        for sector in self.sector_allocations:
            self.sector_allocations[sector] = (
                self.sector_allocations[sector] / total_value
            )

    def check_sector_limits(
        self, max_sector_allocation: float = 0.40
    ) -> list[dict[str, Any]]:
    ...
    def adjust_limits_dynamic(
        self, market_volatility: float, recent_drawdown: float
    ) -> dict[str, float]:
        ...
        # Apply to key limits
        adjusted_limits["max_position_size"] = (
            self.config.get("max_position_size", 0.2) * adjustment_factor
        )
        adjusted_limits["max_leverage"] = (
            self.config.get("max_leverage", 1.0) * adjustment_factor
        )
        adjusted_limits["daily_loss_limit"] = (
            self.config.get("daily_loss_limit", 0.02) * adjustment_factor
        )
    ...
    def calculate_marginal_risk(
        ...
        for existing_symbol, pos_data in current_positions.items():
            if (
                existing_symbol in self.correlation_matrix.index
                and symbol in self.correlation_matrix.index
            ):
    ...
    def check_correlation_risk(
        self, correlation_matrix: pd.DataFrame, positions: list
    ) -> bool:
        """Check if correlation between positions is within limits."""
        max_corr = (
            self.risk_limits.get("max_correlation", 0.7)
            if isinstance(self.risk_limits, dict)
            else getattr(self.risk_limits, "max_correlation", 0.7)
        )
        ...
        for i, pos1 in enumerate(positions):
            for _j, pos2 in enumerate(positions[i + 1 :], i + 1):
                if (
                    pos1 in correlation_matrix.index
                    and pos2 in correlation_matrix.index
                ):
    ...
    def check_concentration_risk(self, sector_allocation: dict) -> bool:
        ...
        max_concentration = (
            self.risk_limits.get("max_concentration", 0.40)
            if isinstance(self.risk_limits, dict)
            else getattr(self.risk_limits, "max_concentration", 0.40)
        )
    ...
    def calculate_var(
        self, returns: pd.Series, confidence_level: float, portfolio_value: float
    ) -> float:
    ...
    def pre_trade_risk_check(
        self, new_trade: dict, current_positions: dict, portfolio_value: float
    ) -> dict:
        """Comprehensive pre-trade risk validation."""
        checks_list = []
        all_passed = True
        ...
        position_size_ok = self.check_position_size(
            new_trade.get("symbol", ""), new_value, portfolio_value
        )
        checks_list.append(
            {
                "check": "position_size",
        ...
        total_tech_value = sum(
            pos.get("value", 0)
            for pos in updated_positions.values()
    ...
    def calculate_risk_metrics_dict(
        self, positions: dict, market_data: dict, portfolio_value: float
    ) -> dict:
        ...
        if not volatilities:
            volatilities = dict.fromkeys(positions.keys(), 0.02)
        ...
        if isinstance(self.risk_limits, dict):
            ...
            max_var = getattr(self.risk_limits, "max_var_95", None) or getattr(
                self.risk_limits, "max_single_loss", 0.02
            )
        ...
        for scenario_name, shocks in scenarios.items():
            ...
            if detailed_mode:
                ...
                results[scenario_name] = {
                    ...
                    "percentage_impact": scenario_pnl / total_value
                    if total_value > 0
                    else 0,
    ...
    def generate_risk_report(
        self, positions: dict, portfolio_value: float, daily_pnl: float
    ) -> dict:
        """Generate comprehensive risk report."""
        # Calculate current metrics
        if positions:
            market_data = {
                "volatilities": dict.fromkeys(positions, 0.02)
            }  # Dummy volatilities
            risk_metrics = self.calculate_risk_metrics_dict(
                positions, market_data, portfolio_value
            )
        else:
            risk_metrics = {}
    ...
    def adjust_risk_limits(
        self, base_limits: Any, market_volatility: float, recent_performance: float
    ) -> Any:
        """Dynamically adjust risk limits based on market conditions."""
        ...
        if hasattr(base_limits, "__dict__"):
            ...
            for field in ["max_position_size", "max_leverage", "max_portfolio_risk"]:
                ...
                if base_value:
                    ...
                    adjustment = (
                        1.0 - (market_volatility - 0.02) * 2
                    )  # Reduce by 2x excess vol
                    adjustment = max(
                        0.5, min(1.0, adjustment)
    ...
    def calculate_margin_requirements(
        self, positions: dict, margin_rates: dict
    ) -> float:
    ...
    def pre_trade_check(
        ...
        portfolio_value = (
            self.portfolio_value if hasattr(self, "portfolio_value") else 100000
        )

        # Perform risk check
        result = self.pre_trade_risk_check(
            new_trade, current_positions, portfolio_value
        )
...
class RiskLimits:
    ...
    def validate_order_params(self, order: Any) -> bool:
        """Validate order parameters for risk compliance."""
        # Basic parameter validation
        if not hasattr(order, "symbol") or not order.symbol:
            logger.warning("Order missing symbol")
            return False

        if not hasattr(order, "quantity") or order.quantity <= 0:
            logger.warning(
                f"Invalid order quantity: {getattr(order, 'quantity', 'N/A')}"
            )
            return False

        if not hasattr(order, "side") or order.side not in ["BUY", "SELL"]:
            logger.warning(f"Invalid order side: {getattr(order, 'side', 'N/A')}")
            return False
    ...
    async def validate_order_risk(self, order: Any) -> bool:
        """Validate order against risk limits (async version)."""
        # This is the async version called by _validate_order_risk
        return self.validate_order_risk_sync(order)

    def validate_order_risk_sync(self, order: Any) -> bool:
        """Validate order against risk limits (sync version)."""
        try:
            # Get current portfolio value
            portfolio_value = (
                getattr(self.portfolio, "current_equity", 0) if self.portfolio else 0
            )
            if portfolio_value <= 0:
                logger.warning("Cannot validate order: invalid portfolio value")
                return True  # Allow if we can't determine portfolio value

            # Calculate order value
            order_value = order.quantity * getattr(order, "price", 0)
            if order_value <= 0:
                logger.warning("Cannot calculate order value")
                return True
            ...
            if position_pct > max_position_size:
                logger.warning(
                    f"Order exceeds position size limit: {position_pct:.1%} > {max_position_size:.1%}"
                )
                return False

            # Check daily loss limit
            if hasattr(self, "daily_pnl") and self.daily_pnl:
                current_daily_loss = min(0, sum(self.daily_pnl))
                max_daily_loss = portfolio_value * self.config.get(
                    "max_daily_loss", 0.02
                )

                if abs(current_daily_loss) > max_daily_loss:
                    logger.warning(
                        f"Daily loss limit exceeded: ${abs(current_daily_loss):.2f} > ${max_daily_loss:.2f}"
                    )
                    return False

            # Check leverage
            if hasattr(self, "calculate_current_leverage"):
                current_leverage = self.calculate_current_leverage()
                max_leverage = self.config.get("max_leverage", 1.0)

                if current_leverage > max_leverage:
                    logger.warning(
                        f"Leverage limit exceeded: {current_leverage:.2f}x > {max_leverage:.2f}x"
                    )
                    return False
            ...
            if hasattr(order, "side") and order.side == "BUY":
                available_cash = (
                    getattr(self.portfolio, "cash", 0) if self.portfolio else 0
                )
                required_cash = order_value

                if required_cash > available_cash:
                    logger.warning(
                        f"Insufficient buying power: ${required_cash:.2f} > ${available_cash:.2f}"
                    )
                    return False
            ...
            # In case of error, allow the order but log it
            return True
