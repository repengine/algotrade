...
class PaperExecutor(BaseExecutor):
    ...
    async def submit_order(self, order: Order) -> str:
        ...
        if order.side == OrderSide.BUY:
            ...
            if estimated_cost > self.buying_power:
                ...
                raise ValueError(
                    f"Insufficient buying power: {self.buying_power:.2f} < {estimated_cost:.2f}"
                )
        ...
        logger.info(
            f"Order submitted: {order.order_id} - {order.side.value} {order.quantity} {order.symbol}"
        )
    ...
    def _calculate_fill_price(
        ...
        slippage_mult = (
            1 + self.slippage if order.side == OrderSide.BUY else 1 - self.slippage
        )
    ...
    async def _execute_fill(self, order: Order, fill_price: float) -> None:
        ...
        if order.side == OrderSide.BUY:
            ...
            realized_pnl = (
                fill_price - pos.average_cost
            ) * order.quantity - self.commission
    ...
    def _estimate_order_cost(self, order: Order) -> float:
        ...
        if order.order_type == OrderType.MARKET:
            ...
            price = (
                max(current_price, order.stop_price)
                if order.side == OrderSide.BUY
                else current_price
            )
        else:
