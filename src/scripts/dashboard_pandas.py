...
def mock_validator(config):
    ...
    for key, value in converted.items():
        ...
        if any(
            x in key
            for x in ["threshold", "oversold", "overbought", "pct", "mult", "ratio"]
        ):
            try:
                converted[key] = float(value)
            except:
        ...
        if self.av_key:
            try:
                ...
                self.av_fetcher = AlphaVantageFetcher(
                    api_key=self.av_key, premium=premium
                )
                ...
                with open(secrets_path) as f:
                    ...
                    if secrets and "data_providers" in secrets:
                        if "alphavantage" in secrets["data_providers"]:
                            return secrets["data_providers"]["alphavantage"].get(
                                "api_key"
                            )
        except Exception as e:
        ...
        if data_source == "alpha_vantage" and self.av_fetcher:
            try:
                ...
                if not df.empty:
                    ...
                    st.warning(
                        "No data from Alpha Vantage, falling back to Yahoo Finance"
                    )
            except Exception as e:
...
class PandasStrategyManager:
        ...
        for py_file in strategies_dir.glob("*.py"):
            ...
            try:
                ...
                spec = importlib.util.spec_from_file_location(
                    f"strategies.{module_name}", py_file
                )
                ...
                if not any(
                    skip in error_msg for skip in ["statsmodels", "sklearn", "scipy"]
                ):
    ...
    def run_backtest(
        ...
        try:
            ...
            # Convert data format
            strategy_data = self.converter.dashboard_to_strategy(data, symbol=symbol)
            ...
            print(
                f"   Processing {len(strategy_data) - lookback} bars (lookback={lookback})"
            )
            signals_count = 0
            ...
            for i in range(lookback, len(strategy_data)):
                ...
                RiskContextMock(account_equity=capital)
            ...
            if not equity_df.empty:
                ...
                total_return = (
                    equity_df["equity"].iloc[-1] / initial_capital - 1
                ) * 100
                ...
                results = {
                    ...
                    "signals": (
                        pd.DataFrame(all_signals) if all_signals else pd.DataFrame()
                    ),
...
def run_monte_carlo_simulation(
    trades_df: pd.DataFrame, initial_capital: float, n_simulations: int = 1000
) -> dict[str, Any]:
...
def detect_market_regimes(data: pd.DataFrame, lookback: int = 20) -> pd.DataFrame:
    ...
    # Ranging regime
    data["price_range"] = (
        data[close_col].rolling(lookback).max()
        - data[close_col].rolling(lookback).min()
    )
...
def run_walk_forward_analysis(
    ...
    min_window_size = (
        lookback_period * 2
    )  # Need at least 2x lookback for meaningful results
    ...
    for i in range(n_windows):
        ...
        if (
            len(oos_data_with_context) < lookback_period + 10
        ):  # Need some bars after lookback
            print(
                f"Window {i+1}: OOS data too small ({len(oos_data_with_context)} bars), skipping"
            )
            continue

        print(
            f"Window {i+1}: IS={len(is_data)} bars, OOS={window_end - is_end} bars (with {lookback_period} lookback bars)"
        )
...
def create_performance_chart(
    results: dict[str, Any], data: pd.DataFrame, initial_capital: float = 100000
) -> go.Figure:
    ...
    if len(backtest_data) > 0:
        ...
        for date in equity_curve.index:
            ...
            if date in backtest_data.index:
                buy_hold_aligned[date] = (
                    backtest_data.loc[date, close_col] * shares_bought
                )
            else:
                # Find nearest date
                ...
                buy_hold_aligned[date] = (
                    backtest_data.iloc[closest_idx][close_col] * shares_bought
                )
    ...
    fig.update_layout(
        title=title_text, showlegend=True, height=800, hovermode="x unified"
    )
...
def main():
    st.set_page_config(
        page_title="AlgoStack Dashboard - Pandas Edition", page_icon="📊", layout="wide"
    )
    ...
    period = st.sidebar.selectbox(
        "Time Period", ["1mo", "3mo", "6mo", "1y", "2y", "5y"], index=3
    )
    ...
    if data_source == "alpha_vantage":
        interval = st.sidebar.selectbox(
            "Interval", ["1m", "5m", "15m", "30m", "60m", "1d"], index=5
        )
    ...
    selected_strategy_name = st.sidebar.selectbox(
        "Select Strategy", strategy_names, format_func=strategy_manager.get_display_name
    )
    ...
    for category, params in param_categories.items():
        if params or category in ["Core", "Risk"]:
            with st.sidebar.expander(
                f"{category} Parameters", expanded=(category == "Core")
            ):
                for param in params:
    ...
    initial_capital = st.sidebar.number_input(
        "Initial Capital ($)", value=100000, step=10000, min_value=1000
    )
    ...
    if st.sidebar.button("🚀 Run Backtest", type="primary"):
        ...
        if "error" in results:
            ...
            with col1:
                ...
                st.metric(
                    "Total Return", f"{total_return:.2f}%", delta=f"{total_return:.2f}%"
                )
            ...
            if "trades" in results and not results["trades"].empty:
                ...
                with col1:
                    avg_win = (
                        trades_df[trades_df["pnl"] > 0]["return"].mean() * 100
            ...
            if (
                enable_monte_carlo
                and "trades" in results
                and not results["trades"].empty
            ):
                st.subheader("📊 Monte Carlo Analysis")

                with st.spinner(f"Running {n_simulations} Monte Carlo simulations..."):
                    mc_results = run_monte_carlo_simulation(
                        results["trades"], initial_capital, n_simulations
                    )
                ...
                if mc_results:
                    ...
                    with col3:
                        st.metric(
                            "Win Probability", f"{mc_results['win_probability']:.1f}%"
                        )
                    with col4:
                        st.metric(
                            "95% Confidence",
                ...
                with st.spinner("Analyzing market regimes..."):
                    ...
                    with col2:
                        ...
                        fig_regime.update_layout(
                            title="Market Regime Distribution", height=300
                        )
                    ...
                    if (
                        ...
                        for _, trade in trades_df.iterrows():
                            ...
                            if entry_time in data_with_regimes.index:
                                entry_regime = data_with_regimes.loc[
                                    entry_time, "regime"
                                ]
                            else:
                                # Find nearest date
                                ...
                                entry_regime = data_with_regimes.iloc[closest_idx][
                                    "regime"
                                ]
                        ...
                        regime_summary = (
                            ...
                            .agg({"return": ["mean", "count", "sum"], "pnl": "sum"})
                            .round(2)
                        )
                        ...
                        fig_regime_perf.add_trace(
                            go.Bar(
                                ...
                                y=avg_returns,
                                text=[
                                    f"{ret:.2f}%<br>({cnt} trades)"
                                    for ret, cnt in zip(avg_returns, trade_counts)
                                ],
                                textposition="auto",
                        ...
                        st.info(
                            "No trades to analyze by regime. Try adjusting strategy parameters to generate more trades."
                        )
            ...
            if enable_walk_forward:
                ...
                with st.spinner(
                    f"Running walk-forward analysis with {walk_forward_windows} windows..."
                ):
