...
for config in configs_to_test:
    ...
    for i in range(config["lookback_hours"], len(hourly_data)):
        ...
        if position == 0:
            ...
            if (
                current_z < -config["zscore_threshold"]
...
if best_config:
    ...
    sell_trades = [
        t for t in best_config["trades"] if t["action"] in ["SELL", "STOP_LOSS"]
    ]
    ...
    if len(buy_trades) > 0 and len(sell_trades) > 0:
        ...
        if len(trade_returns) > 1:
            trade_sharpe = (
                np.sqrt(252 * 6.5) * np.mean(trade_returns) / np.std(trade_returns)
            )
        ...
        for trade in best_config["trades"][-10:]:
            print(
                f"  {trade['action']} at {trade['time']}: ${trade['price']:.2f} "
